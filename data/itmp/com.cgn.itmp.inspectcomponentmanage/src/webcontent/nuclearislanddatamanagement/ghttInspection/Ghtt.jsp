...
<body id="ghtt" style="width: 100%; height: 100%;">
	<div
		...
		<button class="btn btn-default" id="localsavle" onclick="saveImg()">保存为附件</button>
...
<script>
	...
	function setFormData(data) {
		...
		if (data.record.pipeType == "跟踪管") {
			...
			steamGeneratorModel = data.record.datas.steamGeneratorModel;
			steamGeneratorNo = data.record.datas.steamGeneratorNo;
			checkRange = data.record.datas.checkRange;
			$("#websavle").show();
			$("#localsavle").hide();
			uuid = data.record.datas.uuid;
			overhaulRotation = data.record.datas.overhaulRotation;
			steamGeneratorNo = data.record.datas.steamGeneratorNo;
			if (data.record.datas.oneEighthDisplace) {
				datas.statisticstype = data.record.datas.checkPlan + "＋"
						+ data.record.datas.oneEighthDisplace;
				datas.jd = getCookie('jdid');
				datas.model = data.record.datas.steamGeneratorModel;
				var json = nui.encode({
					myDatas : datas,
				});
				getPipePoint(json);
		...
		$.ajax({
			...
			if(getCookie('jdid')=="86"){
				...
				for (var i = 0; i < seriesArray.length; i++) {
					...
					if (seriesArray[i].name == '被检查') {
						selectpipe = seriesArray[i].data.length;
					}
				}
			}
	...
	//从后台获取管版图坐标
	function getPipePoint(json) {
		$
				.ajax({
					url : "com.cgn.itmp.inspectcomponentmanage.pipelayout.pipelayoutbiz.getPipePoint.biz.ext",
					type : 'POST',
					data : json,
					cache : false,
					contentType : 'text/json',
					success : function(data) {
						...
						;
						myChart.setOption(
								initEchartsData(seriesArray, 'right'), true);
					}
				});
	}
	//图表赋值
...
// 	            	color:'#ddd',
			...
			title : [
					...
					{
						text : '大修轮次：' + overhaulRotation + ' 检查范围：'
								+ checkRange + ' 总管束：' + allpipe + ' 计划数：'
								+ selectpipe + ' 百分比：'
...
// 				inverse : postionOfy == "left" ? false : true,
	...
	function saveImg() {
		html2canvas($("#main")).then(
				function(canvas) {
					var imgUri = canvas.toDataURL("image/png").replace(
							"image/png", "image/octet-stream"); // 获取生成的图片的url
					var aLink = document.createElement("a");
					aLink.style.display = "none";
					aLink.href = imgUri;
					//基地+机组+大修轮次+蒸发器编号+“管版图”
					aLink.download = getCookie('jdname') + "_" + getCookie('jzname') + "_"
							+ overhaulRotation + "_" + steamGeneratorNo
							+ "管版图.png";
					// 触发点击-然后移除
