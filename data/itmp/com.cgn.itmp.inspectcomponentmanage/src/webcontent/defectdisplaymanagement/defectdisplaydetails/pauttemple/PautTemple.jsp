...
<!-- 		<input id='sortfield' class="nui-hidden" name="defectsshowdetailedinformation.sortfield"/> -->
		...
		<table style="width: 100%; height: 100%; table-layout: fixed;"
			...
			<tr>
				...
				<td colspan="1"><input class="nui-textbox"
					name="defectsshowdetailedinformation.defecttype" emptyText="请输入大修轮次" requiredErrorText="缺陷类型不能为空" required="required" style="width: 100%" /></td>
			</tr>
			<tr>
				<td class="form_label">缺陷起始位置（mm）</td>
				<td colspan="1"><input class="nui-textbox"
					name="defectsshowdetailedinformation.defectstartlocation" emptyText="请输入缺陷起始位置" requiredErrorText="缺陷起始位置不能为空" required="required" style="width: 100%" /></td>
				<td class="form_label">缺陷长度（mm）</td>
				<td colspan="1"><input class="nui-textbox"
					name="defectsshowdetailedinformation.defectlength" vtype="float;range:0,99999" floatErrorText="请输入数字" rangeErrorText="请输入正数" style="width: 100%" /></td>
				
			</tr>
			<tr>
				<td class="form_label">缺陷高度（mm）</td>
				<td colspan="1"><input class="nui-textbox"
					name="defectsshowdetailedinformation.defectheight" vtype="float;range:0,99999" floatErrorText="请输入数字" rangeErrorText="请输入正数" style="width: 100%" /></td>
				<td class="form_label">波幅高度（%）</td>
				<td colspan="1">
				<input name="defectsshowdetailedinformation.amplitudeheight" vtype="float;range:0,99999" floatErrorText="请输入数字" rangeErrorText="请输入正数" class="nui-textbox" style="width: 100%" />
			</tr>
			<tr>
...
<td colspan="1"><input class='nui-dictcombobox' dictTypeId='ISORNO' nullItemText="请选择是否超标缺陷.." required="required" requiredErrorText="是否超标缺陷不能为空" name="defectsshowdetailedinformation.influence" required="required" style="width: 100%" /></td>			
			...
			<td class="form_label">备注</td>
				<td colspan="3"><input class="nui-textarea" 
					name="defectsshowdetailedinformation.templeremark" style="width: 98%" />
	...
	<script type="text/javascript">
					...
					url:"/itmp/inspectcomponentmanage/defectdisplaymanagement/defectdisplaydetails/HistoryNumCheck.jsp",
					  title: "历史编号选择", width: 516, height:300,
					   ...
					   var iframe = this.getIFrameEl();
		                    var data = {stencilplate:"paut"};//传入页面的json数据
		...
		function saveData() {
			...
			data.defectsshowdetailedinformation.stencilplate="paut";
...
// 				nui.get('sortfield').setValue(json.defectsshowdetailedinformation.sortfield);
			...
			//如果是点击编辑类型页面
			if (infos.pageType == "edit" || infos.pageType == "paut") {
