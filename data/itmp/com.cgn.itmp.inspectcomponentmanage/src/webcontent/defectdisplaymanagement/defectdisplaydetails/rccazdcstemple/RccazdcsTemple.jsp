...
- Description:RCCA自动超声模板信息实体录入页面
    --%>
<head>
<title>RCCA自动超声</title>
		...
		<table style="width: 100%; height: 100%; table-layout: fixed;"
			...
			<tr>
				...
				<td colspan="1"><input class="nui-textbox" id="bjmc" name="defectsshowdetailedinformation.componentname"
					 readOnly="readOnly" style="width: 100%" required="required" emptyText="选择被检部位自动回填" requiredErrorText="部件名称不能为空"/></td>
			...
			<tr>
				...
				<td colspan="1"><input class="nui-textbox"
					name="defectsshowdetailedinformation.defecttype" emptyText="选输入缺陷类型" required="required" requiredErrorText="缺陷类型不能为空" style="width: 100%" /></td>
			</tr>
			</table>
			<fieldset id="baseInfo" style="width:100%;">
				<legend><span>显示值</span></legend>
					<table style="width: 100%; height: 100%; table-layout: fixed;"
						class="nui-form-table">
				                <tr>
				                    <td class="form-label">最小厚度(mm)</td>
				                    <td class="form-label">截面损失(%)</td>
				                    <td class="form-label" rowspan="2"> 轴向位置(mm)</td>
				                   
				                </tr>
				                <tr>
				                    <td class="form-label">幅值(V)</td>
				                    <td class="form-label">相位(°)</td>
				                 </tr>
				                 <tr>
				                    <td >
				                        <input class="nui-textbox" 
				                        vtype="float;range:0,99999" floatErrorText="请输入数字" rangeErrorText="请输入正数"
				                        style="width: 100%" name="defectsshowdetailedinformation.amplitude"/>
				                    </td>
				                    <td >
				                        <input class="nui-textbox" 
				                        vtype="float;range:0,99999" floatErrorText="请输入数字" rangeErrorText="请输入正数"
				                        style="width: 100%" name="defectsshowdetailedinformation.phase"/>
				                    </td>
				                    <td >
				                        <input class="nui-textbox" 
				                        vtype="float;range:0,99999" floatErrorText="请输入数字" rangeErrorText="请输入正数"
				                        style="width: 100%" name="defectsshowdetailedinformation.axialposition" />
				                    </td>
				                </tr>
						</table>
				</fieldset>
...
<td colspan="1"><input class='nui-dictcombobox' dictTypeId='ISORNO' nullItemText="请选择是否超标缺陷.." required="required" requiredErrorText="是否超标缺陷不能为空" name="defectsshowdetailedinformation.influence" required="required" style="width: 100%" /></td>				<td class="form_label"></td>
	...
	<script type="text/javascript">
					...
					url:"/itmp/inspectcomponentmanage/defectdisplaymanagement/defectdisplaydetails/HistoryNumCheck.jsp",
					  title: "历史编号选择", width: 516, height:300,
					   ...
					   var iframe = this.getIFrameEl();
		                    var data = {stencilplate:"rccazdcs"};//传入页面的json数据
		...
		function saveData() {
			...
			data.defectsshowdetailedinformation.stencilplate="rccazdcs";
...
// 				nui.get('sortfield').setValue(json.defectsshowdetailedinformation.sortfield);
			...
			//如果是点击编辑类型页面
			if (infos.pageType == "edit" || infos.pageType == "rccazdcs") {
