...
<body style="background-color:#f1f1f1;width: 98%; height: 97%;">
				...
				<div style="width: 100%; height: 100%;">
					...
					<div id="form1" class="nui-form displaynone" align="center"
						...
						<input class="nui-hidden" name="criteria/_entity"
							value="com.cgn.itmp.inspectcomponentmanage.foreignmattermg.Foreignmattermg">
						<!-- 排序字段 -->
						<table id="table1" class="table" style="height: 100%">
							...
							<tr
								...
								<td style="visibility: hidden; height: 1px; overflow: hidden;"
									...
									name="criteria/_expr[7]/isdel" value="0" /> <input
	...
	<div style="width: 100%; height: 94%;" showToolbar="false"
		...
		<div class="nui-fit">
			<div id="datagrid1" dataField="foreignmattermgs" class="nui-datagrid"
				style="width: 100%; height: 100%;"
				url="com.cgn.itmp.inspectcomponentmanage.foreignmattermg.foreignmattermgbiz.queryForeignmattermgs.biz.ext"
				...
				<div property="columns">
					...
					<div field="funposition" headerAlign="center" allowSort="true">
						功能位置</div>
					<div field="focde" headerAlign="center" allowSort="true" renderer="renderFocde">
						FO编号</div>
					...
					<div field="foattribute" headerAlign="center" allowSort="true">
						FO属性</div>
					<div field="fodisimgfj" headerAlign="center" allowSort="true"
						renderer="setFile1">FO分布图</div>
					<div field="fj" headerAlign="center" allowSort="true"
						renderer="setFile2">相关图片</div>
					<div field="remake" headerAlign="center" allowSort="true">备注</div>
					<div field="processStatus" headerAlign="center" visible="false"  allowSort="true"></div>
				</div>
			</div>
		</div>
	</div>
...
<%-- 		accessControlButton('<%=userRoleCodeList%>',7); --%>
		...
		formData["criteria/_expr[11]/isdel"] = "0";
		formData["criteria/_expr[12]/jz"] = getCookie('jzid');
		formData["criteria/_expr[13]/jd"] = getCookie('jdid');
		...
		function renderFocde(e){
			return '<a style="cursor: pointer" onclick=show("'+e.record._uid+'")>'+e.record.focde+'</a>';
		}
		function show(uid){
				var record = grid.getRowByUID(uid);
				nui.open({
					showMaxButton : true,
                    url : "/itmp/inspectcomponentmanage/foreignmattermg/ForeignmattermgForm.jsp",
                    title: "查看记录", width: 655, height: 350,
                    onload: function () {//弹出页面加载完成
                    var iframe = this.getIFrameEl();
                    var data = {pageType:"show",record:{foreignmattermg:record}};
								...
								function(action) {
									if (action == "ok") {
										var json = nui
												.encode({
													data : rows,
													entity : "com.cgn.itmp.inspectcomponentmanage.foreignmattermg.Foreignmattermg",token:"<%=TokenUtil.getTokenString(request.getSession())%>"
												});
		...
		function refresh() {
			...
			json["criteria/_expr[11]/isdel"] = "0";
		...
		function recycle() {
			nui
					.open({
						showMaxButton : true,
						url : "/itmp/common/recyclebin/recyclebinattachFore.jsp",
						title : "回收站",
						width : 1000,
						height : 570,
						onload : function() {//弹出页面加载完成
							...
							var data = {
								pageType : "recyclebin",
								urlName : "com.cgn.itmp.inspectcomponentmanage.foreignmattermg.foreignmattermgbiz.queryForeignmattermgs.biz.ext",//回收
								entityVal : "com.cgn.itmp.inspectcomponentmanage.foreignmattermg.Foreignmattermg",
								...
								columnscode : [ {
									...
									field : "fodisimgfileid",
									width : 100,
									headerAlign : "center",
									allowSort : true,
									header : "fodisimgfileid",
									visible : false
								}, {
									field : "fileid",
									width : 100,
		...
		function batchAdd() {
			nui
					...
					$("#delebtn").prop("disabled", "disabled");
							...
							var data = {
								pageType : "batchAdd",
								tempFileName : "foreignmattermg.xlsx",
								tempLoadName : "异物管理",
								entitfName : "com.cgn.itmp.inspectcomponentmanage.foreignmattermg.Foreignmattermg"
							};//传入页面的json数据
		...
		function output() {
			...
			nui.open({
						...
						onload : function() {//弹出页面加载完成
							var iframe = this.getIFrameEl();
							var data = {
								entityName : "com.cgn.itmp.inspectcomponentmanage.foreignmattermg.Foreignmattermg",
		...
		function record() {
			nui
					.open({
						showMaxButton : true,
						url : "/itmp/inspectcomponentmanage/foreignmattermg/Foreignmattermg_mfList.jsp",
						title : "修改记录",
						width : 1200,
						height : 630,
					});
...
// 					.open({
// 						showMaxButton : true,
// 						url : "/itmp/inspectcomponentmanage/foreignmattermg/StatisticsAnalysis.jsp",
// 						title : "数据统计与分析",
// 						width : "90%",
// 						height : "90%",
// 					});
// 		}
		function analysis(){
				addTab("","数据统计与分析","/itmp/inspectcomponentmanage/foreignmattermg/StatisticsAnalysis.jsp",null);
		}
		//到期提醒
    	function addTab(position,type,url,data) {
		 ...
		 function onvalueChangedfo(e){
            	var value=e.value;
            	nui.get('fokg').setValue(value);
            } 
            //遍历所选数据
          function fordata(){
          	...
          	if(data){
          		for(var i=0;i<data.length;i++){
          			if(data[i].processStatus=="0"){
		          		focds+="FO编号为["+data[i].focde+"]正在[修改]流程中<br>";
          			}
          		}
          	}
          	if(focds==""){
