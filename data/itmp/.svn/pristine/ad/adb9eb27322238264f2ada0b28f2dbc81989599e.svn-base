 ...
 * $log$
 */
public class ExecutiveEntriesSNJHDXServiceImpl extends DASDaoSupport implements
		ExecutiveEntriesSNJHDXService {

	/**
	 * 
	 */
	public ExecutiveEntriesSNJHDXServiceImpl() {
		// TODO 自动生成的构造函数存根
	}
	...
	public DataObject[] getProtypeAndPartsData(String baseid,String crewid) {
		// TODO 自动生成的方法存根
		String address = "com.cgn.itmp.outlineandplan.outline.common.dxzxtm.getProtype5";
	...
	public void addInfoMation(DataObject[] data) {
		...
		DataObject temp  = DataObjectUtil.convertDataObject(data[0], "com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandOverhaul.OverhaulEntity", false);
		temp.setString("id", uuid);
		DatabaseExt.insertEntityCascade("default", temp);
		String protype = temp.getString("protype");
		this.addRecordInfoMation(temp,"新增十年计划（核岛部件）:"+protype+"_大修执行条目1条","新增",uuid);
	}
	...
	public void deleteRecoversData(DataObject[] data) {
		...
		if(!StringHelper.isEmpty(ids)){
			...
			for (int i = 0; i < idArr.length; i++) {
				String id = idArr[i];
				String address = "com.cgn.itmp.outlineandplan.outline.common.dxzxtm.deleteRecoversData5";
	...
	public void addEditionInfoMation(DataObject[] data){
		if(data == null) return;
		String uuid = UUID.randomUUID().toString().replaceAll("-", "");
		DataObject temp  = DataObjectUtil.convertDataObject(data[0], "snjh.hd.dx.ascendingedition.Dgglsnjhhdedition", false);
		temp.setString("id", uuid);
		DatabaseExt.insertEntityCascade("default", temp);
//		String number = this.getCountForEdition();
		this.addRecordInfoMation(temp,"十年计划（核岛部件）由A版升为B版","升版",uuid);
	}
	...
	public String getCountForEdition() {
		// TODO 自动生成的方法存根
		String address = "com.cgn.itmp.outlineandplan.outline.common.dxzxtm.getCountForEdition5";
	...
	public void addRecordInfoMation(DataObject temp,String content,String type,String uuid){
		...
		temp1.setString("ident", "十年核岛大修执行条目");
	...
	public void updateInfoMation(DataObject data) {
		...
		DataObject temp = DataObjectUtil.convertDataObject(data, "com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandOverhaul.OverhaulEntity", false);
		...
		if(!StringHelper.isEmpty(parms)){
			if("remove".equals(parms)){
				this.addRecordInfoMation(temp,"删除十年计划（核岛部件）:"+protype+"_大修执行条目1条","删除",id);
				...
				if(!StringHelper.isEmpty(updata)){
					DataObject dataEntity = DataObjectUtil.createDataObject("com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandOverhaulEdit.NuclearIslandEdit");
					...
					while(keys.hasNext()){
						...
						Field modeField = new ViewModelHelper().getField("com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandOverhaul.OverhaulEntity", key);
						if(modeField != null){
				...
				this.addRecordInfoMation(temp,"修改十年计划（核岛部件）:"+protype+"_"+sb.toString()+"","修改",uuid);
			}
		}
	...
	public Map<String,Object> synchronous(DataObject data){
		...
		Object[] typeDataObject = (Object[])DatabaseExt.queryByNamedSql("default", "com.cgn.itmp.outlineandplan.outline.common.dxzxtm.getDGDXdata", parmsMap);
		...
		if(typeDataObject != null && typeDataObject.length>0){
			...
			if(stateMap != null && stateMap.size()>0){
				DataObject dataEntity = DataObjectUtil.createDataObject("com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandOverhaul.OverhaulEntity");
				for (int i = 1; i <= 60; i++) {
