 ...
 * $log$
 */
public class ExecutiveEntriesSNJHCGDRCServiceImpl extends DASDaoSupport
		implements ExecutiveEntriesSNJHCGDRCService {

	/**
	 * 
	 */
	public ExecutiveEntriesSNJHCGDRCServiceImpl() {
		// TODO 自动生成的构造函数存根
	}
	...
	public DataObject[] getProtypeAndPartsData(String baseid,String crewid) {
		// TODO 自动生成的方法存根
		String address = "com.cgn.itmp.outlineandplan.outline.common.dxzxtm.getProtype8";
	...
	public void addInfoMation(DataObject[] data) {
		...
		DataObject temp  = DataObjectUtil.convertDataObject(data[0], "com.cgn.itmp.outlineandplan.tenyearplan.ConventionalIslandDaily.DailyEntity", false);
		temp.setString("id", uuid);
		DatabaseExt.insertEntityCascade("default", temp);
		String protype = temp.getString("protype");
		this.addRecordInfoMation(temp,"新增十年计划（常规岛及BOP）:"+protype+"_日常执行条目1条","新增",uuid);
	}
	...
	public void deleteRecoversData(DataObject[] data) {
		...
		if(!StringHelper.isEmpty(ids)){
			...
			for (int i = 0; i < idArr.length; i++) {
				String id = idArr[i];
				String address = "com.cgn.itmp.outlineandplan.outline.common.dxzxtm.deleteRecoversData8";
...
//		String number = this.getCountForEdition();
	...
	public String getCountForEdition() {
		// TODO 自动生成的方法存根
		String address = "com.cgn.itmp.outlineandplan.outline.common.dxzxtm.getCountForEdition8";
	...
	public void addRecordInfoMation(DataObject temp,String content,String type,String uuid){
		...
		temp1.setString("ident", "十年常规岛及BOP日常执行条目");
	...
	public void updateInfoMation(DataObject data) {
		...
		DataObject temp = DataObjectUtil.convertDataObject(data, "com.cgn.itmp.outlineandplan.tenyearplan.ConventionalIslandDaily.DailyEntity", false);
		...
		if(!StringHelper.isEmpty(parms)){
			if("remove".equals(parms)){
				this.addRecordInfoMation(temp,"删除十年计划（常规岛及BOP）:"+protype+"_日常执行条目1条","删除",id);
				...
				if(!StringHelper.isEmpty(updata)){
					...
					while(keys.hasNext()){
						...
						Field modeField = new ViewModelHelper().getField("com.cgn.itmp.outlineandplan.tenyearplan.ConventionalIslandDaily.DailyEntity", key);
						if(modeField != null){
	...
	public Map<String,Object> synchronous(DataObject data){
		...
		Object[] typeDataObject = (Object[])DatabaseExt.queryByNamedSql("default", "com.cgn.itmp.outlineandplan.outline.common.dxzxtm.getJCJHRCCGDdata", parmsMap);
		if(typeDataObject != null && typeDataObject.length>0){
			...
			if(stateMap != null && stateMap.size()>0){
				DataObject dataEntity = DataObjectUtil.createDataObject("com.cgn.itmp.outlineandplan.tenyearplan.ConventionalIslandDaily.DailyEntity");
