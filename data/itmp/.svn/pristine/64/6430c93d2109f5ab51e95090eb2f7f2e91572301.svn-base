...
<body
		...
		<div class="search-container folding-menu" id="queryForm">
			<div style="width: 100%; height: 100%;">
				...
				<div id="form1" class="nui-form display" align="center" style="height: 27px">
					<!-- 数据实体的名称 -->
					<input class="nui-hidden" name="criteria/_entity" value="com.cgn.itmp.dailyandoverhaul.overhaul.QualityMgt.QualityMgtSearch">
					<!-- 排序字段 -->
					<table id="table1" class="table" style="height: 100%">
						<tr>
							...
							<td colspan="1"><input class="nui-textbox"
									...
									value="all"> <input class="nui-hidden"
									name="criteria/_expr[1]/addtime" /> <input class="nui-hidden"
					...
					<input class="nui-hidden" name="criteria/_expr[2]/isdelete"
									...
									value="end">
									<input
										class="nui-hidden" name="criteria/_orderby[4]/_property"
										value="orderindex"> <input class="nui-hidden"
										name="criteria/_orderby[4]/_sort" value="desc">
				</div>
	...
	<div region="center" style="width: 100%; height: 93%;"
...
<!-- 						</ul> -->
		...
		<div class="nui-fit">
			<div id="datagrid1" dataField="tddxglzlgls" class="nui-datagrid"
				style="width: 100%; height: 100%;"
				url="com.cgn.itmp.dailyandoverhaul.overhaul.QualityMgt.queryTdDxglZlgls.biz.ext"
				...
				allowSortColumn="true" sortMode="client" oncellbeginedit="onCellBeginEdit">
				<div property="columns">
					...
					<div field="type" width="100" headerAlign="center"
						renderer="zltypeRenderer">
						类型 <input property="editor" required="true"
									requiredErrorText="类型不能为空" class="nui-dictcombobox"
							style="width: 100%;" dictTypeId="ZLGL_TYPE" />
					</div>
					<div field="name" allowResize="false" width="120"
						headerAlign="center" allowSort="true">
						名称 <input property="editor" id="name" class="nui-textbox name" readonly="readonly"  style="width: 100%;" />
					</div>

					<div field="bc" allowResize="false" width="120"
						headerAlign="center" allowSort="true">
						版次<input property="editor" id="version" class="nui-textbox" onvalidation="hasIllegalChar(e,50,true,true)" style="width: 100%;" />
					</div>
					<div field="fj" allowResize="false" width="120" renderer="setFile"
						headerAlign="center" allowSort="true">
						附件<input property="editor" class="nui-fileupload nui-form-input" onvalidation="hasIllegalChar(e,50,false,true)"
					...
					<div field="adduser" allowResize="false" width="120"
						headerAlign="center" allowSort="true">
						修改者<input property="editor" class="nui-textbox adduser"
							readonly="readonly" style="width: 100%;" />
					</div>
					<div field="addtime" width="100" allowSort="true"
						dateFormat="yyyy-MM-dd HH:mm:ss">
						日期 <input property="editor" class="nui-datepicker scsj"
	...
	<script type="text/javascript">
			...
			tip1.set({
				...
				onopen: function (e) {      //提示框打开触发事件
				 ...
				 tip1.setContent("请选择需要修改的数据");  //给提示框内容添加值
				} 
			});
			...
			tip.set({
			    ...
			    onopen: function (e) {      //提示框打开触发事件
				 ...
				 tip.setContent("系统默认当前登录人！");  //给提示框内容添加值
			    } 
			});
		 ...
		 function output(){
                ...
                nui.open({showMaxButton : true,
                    ...
                    var data = {
                            entityName:"com.cgn.itmp.dailyandoverhaul.overhaul.QualityMgt.QualityMgtSearch",
                            queryParam:json,								    //查询数据
                            queryKey:'criteria',                              //查询实体名
                            query:['xh','isdelete','jdid','updtime'],  //按顺序放置
                            filterField:['status','bz','xgyy','jdid','jzid','fileRelativePath','orderindex','uuid','isdelete','fj','updtime','upduser','dxlc','fildid','fileId','fileRealName','filePath','fileSize','fileType','fileUploadTime','fileUploadUser','fileNewName'],
                            expTitle:"质量管理",
                            dicFieds : "type@ZLGL_TYPE",
                            jdFieds:"jdid",
                        	jzFieds:"jzid",
			...
			function onCellValidation(e) {
	         if(e.field == "jzid") {
	           ...
	           if (e.field == "jzid"||e.field == "dxlc"||e.field == "type" || e.field == "fj" || e.field == "xh") {
...
//                     grid.beginEditRow(e.row); 
        ...
        function saveData(){
			grid.commitEdit();
				...
				if(jsonObject != null && jsonObject.length > 0){
					jsonObject[0]['filename'] = fileName;
			 		...
			 		jsonObject[0]['addtime'] = nui.formatDate(jsonObject[0]['addtime'],'yyyy-MM-dd HH:mm:ss');
			 		var num=0;
			 		jsonObject[0]['jdid']= getCookie('jdid');
			 		jsonObject[0]['name'] = fileName == ''?jsonObject[0]['name']:fileName.split('.')[0];
				}
			 	var data = {
	            ...
	            nui.ajax({
	                url: "com.cgn.itmp.dailyandoverhaul.overhaul.QualityMgt.newAddEdit.biz.ext",
	                type: 'POST',
	                data: json,
            ...
            function btnQuery(){
				...
				if(gjz==""){
					...
					nui.get("update").disable();
					 ...
					 var data = {
					 	 matchFied :"xh,name,bc,adduser",
					 	 dicFied : "type@ZLGL_TYPE",
						 wordVal : $("#cond").val(),
			...
			function batchAdd(){
			 nui.open({showMaxButton : true,
                    ...
                    onload: function () {//弹出页面加载完成
	                    ...
	                    var data = {
	                    	pageType:"batchAdd",
	                    	tempFileName:"dxzlglImport.xlsx",
	                    	tempLoadName :"质量管理导入模板",
	                    	entitfName:"com.cgn.itmp.dailyandoverhaul.overhaul.QualityMgt.QualityMgt"
	                    };//传入页面的json数据
            ...
            //回收站页面
            function recycle() {///http://127.0.0.1:8080/itmp/dxgcgl/zlcontroller/HuiShouTdDxglZlglList.jsp
                nui.open({showMaxButton : true,//dxgcgl/zlcontroller/HuiShouTdDxglZlglList.jsp
                    url: "/itmp/overhaul/QualityMgt/QualityMgtRecycle.jsp",
              ...
              //类型
            function zltypeRenderer(e){
				return nui.getDictText("ZLGL_TYPE",e.value);
			}
                        ...
                        if(rows.length > 0){
                            ...
                            function(action){
                                if(action=="ok"){                          
                                   var json = nui.encode({
										data : rows,
										entity : "com.cgn.itmp.dailyandoverhaul.overhaul.QualityMgt.QualityMgt",token:"<%=TokenUtil.getTokenString(request.getSession())%>"
									});
