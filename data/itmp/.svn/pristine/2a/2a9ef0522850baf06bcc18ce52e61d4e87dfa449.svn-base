...
<script type="text/javascript">
    ...
    function saveData() {
        ...
        var success = true;
        for (var i = 0; i < json.tcgncgdylgdrc.length; i++) {
            var jsondata = "{'tcgncgdylgdrc':{'parms':'remove','uuid':'" + json.tcgncgdylgdrc[i].uuid + "','isdelete':'1','cname':'" + json.tcgncgdylgdrc[i].checkprojectname + "','currentuser':'" + $('#creator').val() + "','currentdatetime':'" + $('#datetime').val() + "','currentreason':'" + $('#reason').val() + "'}}";
            $.ajax({
                url: "com.cgn.itmp.outlineandplan.specialinspection.ConventionalIslandDaily.CGDYLGD.biz.ext",
                type: 'POST',
                data: jsondata,
                cache: false,
    ...
    /*   var json;
      function saveData(){
                  ...
                  if(rows.length > 0){
                      ...
                      function(action){
                          if(action=="ok"){
                              var json = nui.encode({tcgncgdylgdrcs:rows});
                              grid.loading("正在删除中,请稍等...");
                              $.ajax({
                                  url:"com.cgn.itmp.outlineandplan.specialinspection.tcgncgdylgdrcbiz.ljDeletetcgncgdylgdrcs.biz.ext",
                                  type:'POST',
                                  data:json,
                                  cache: false,
                                  contentType:'text/json',
