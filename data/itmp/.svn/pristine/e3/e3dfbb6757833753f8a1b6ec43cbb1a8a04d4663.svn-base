...
<script type="text/javascript">
    ...
    function saveData() {
        ...
        var success = true;
        for (var i = 0; i < json.tdggltzyjcjhty.length; i++) {
            var jsondata = "{'tdggltzyjcjhty':{'parms':'remove','uuid':'" + json.tdggltzyjcjhty[i].uuid + "','isdelete':'1','cname':'" + json.tdggltzyjcjhty[i].checkprojectname + "','currentuser':'" + $('#creator').val() + "','currentdatetime':'" + $('#datetime').val() + "','currentreason':'" + $('#reason').val() + "'}}";
            $.ajax({
                url: "com.cgn.itmp.outlineandplan.specialinspection.ConventionalIslandOverhaul.LJCGDTY.biz.ext",
                type: 'POST',
                data: jsondata,
                cache: false,
