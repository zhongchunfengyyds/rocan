...
- Description:
    ...
    <body style="width:98%;height:97%;">
        <div class="nui-panel" title="年度计划常规岛及BOP换料大修计划" style="width:100%;height:auto;" showToolbar="false" showFooter="true">
            <div id="form1" class="nui-form" align="left" style="height:100%">
                <!-- 数据实体的名称 -->
                <input class="nui-hidden" name="criteria/_entity" value="com.cgn.itmp.outlineandplan.annual.ConventionalIslandOverhaulPlan.PlanEntity">
                <!-- 排序字段 -->
        ...
        <!--footer-->
         <div title="center" region="center" style="width:100%;height:90%;">
            <div class="nui-fit">
                <div 
                        id="datagrid1"
                        dataField="tgglndjhcgddxs"
                        class="nui-datagrid"
                        style="width:100%;height:100%;"
                        url="com.cgn.itmp.outlineandplan.annual.ConventionalIslandOverhaul.queryTgglndjhcgddxs.biz.ext"
				...
				search();//grid查询
                                ...
                                function createplan(){
                                	...
                                	if(lc == ""){
                                		...
                                		json.crew = jzid;
                                		json.lc = lc;
                                		json.ident = "年度计划常规岛大修";
                                		json.entity = "com.cgn.itmp.outlineandplan.outline.common.curriculumVitae.Dgglvitae10";
                                		...
                                		$.ajax({
							                ...
							                success:function(data){
							                	if(data.record){
							                		var jsondata = '{"data":{"ident":"年度计划常规岛大修","baseid":"'+jdid+'","crewid":"'+jzid+'","lc":"'+lc+'","username":"'+'<%= username%>'+'"}}';
			        								var urlStr = "com.cgn.itmp.outlineandplan.annual.ConventionalIslandOverhaul.createNDJHDX.biz.ext";
							                		...
							                		nui.alert("该轮次大修计划数据已存在,无需重新生成","提示");
							                	}
							                }
							            });
                                	}
					           ...
					           //超链接页面  
					            function showdetail() {
					                ...
					                if (row) {
					                    nui.open({showMaxButton : true,
					                        url: "/itmp/annual/ConventionalIsland/Overhaul/recordList.jsp",
					                        title: "数据详情",
					                        width: 1000,
					                        height: 600,
					                        onload: function () {
			                    ...
			                    function showrecorddetail(){
					                ...
					                if (row) {
					                	nui.open({showMaxButton : true,
					                        url: "/itmp/annual/ConventionalIsland/Overhaul/recodeDetail.jsp",
					                        title: "数据详情",
					                        width: 800,
					                        height: 550,
					                        onload: function () {
					                             var iframe = this.getIFrameEl();
								                 var data = {pageType:"data",record:{ndplan:row.ndplan,crew:row.crew,base:row.base,lc:row.lc,ident:'年度计划常规岛大修'}};
								                //直接从页面获取，不用去后台获取
								                iframe.contentWindow.setFormData(data);
				                            }
			                            });
	...
	if(rows.length > 0){
		...
		function(action){
            if(action=="ok"){
                ...
                json.entity='com.cgn.itmp.outlineandplan.annual.ConventionalIslandOverhaulPlan.PlanEntity';
                json.mxEntity='com.cgn.itmp.outlineandplan.annual.ConventionalIslandOverhaul.PlanGeneration';
				...
				nui.open({
					 showMaxButton : true,
					 url : '/itmp/annual/ConventionalIsland/Overhaul/effectList.jsp',
					 title :'生效',
			 		 width: '80%',
	                 height: '80%',
	                 onload: function () {
		...
		//数据行渲染下载标签                 
		 function downloadFile(e){
          ...
          function expotyPDF(id){
         		var json = nui.encode({
         					id: id,
         					entityName:'com.cgn.itmp.outlineandplan.annual.ConventionalIslandOverhaulPlan.PlanEntity',
         					childEntityName:'com.cgn.itmp.outlineandplan.annual.ConventionalIslandOverhaul.PlanGeneration'
         				});
