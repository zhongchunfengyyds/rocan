    ...
    <body style="width:98%;height:95%;">
        <div class="nui-panel" title="查询"   style="width:100%;height:25%;" showToolbar="false" showFooter="true">
            <div id="recordFromList" class="nui-form" align="center" style="height:100%">
                <!-- 数据实体的名称 -->
                <input class="nui-hidden" name="criteria/_entity" value="com.cgn.itmp.outlineandplan.outline.common.curriculumVitae.Dgglvitae6">
                <!-- 排序字段 -->
                <!-- 排序字段 -->
			...
			nui.get('jz').setValue(jzid);
			                    ...
			                    function showdetail() {
			                    	...
			                    	if(row.type == '新增' || row.type == '修改'){
			                    		urlStr = "/itmp/tenyearplan/ConventionalIsland/Overhaul/addForm.jsp?flag=1";
			                    		if (row) {
			                    		 nui.open({showMaxButton : true,
									        ...
									        onload: function () {
									        	...
									        	var parms = '{"parms":{"id":"'+row.planid+'","name":"","entity":"com.cgn.itmp.outlineandplan.tenyearplan.ConventionalIslandOverhaul.OverhaulEntity"}}';
										        ...
										        $.ajax({
											        ...
											        success:function(data){
											        	...
											        	if(returnJson.record[0]){
											        		var data = {pageType:"view",record:{tdgglsnjhcgddx:returnJson.record[0]}};
								                            //直接从页面获取，不用去后台获取
                                		...
                                		urlStr = "/itmp/tenyearplan/ConventionalIsland/Overhaul/vitaeList.jsp";
                                		if (row) {
	                                		 nui.open({showMaxButton : true,
										        url: urlStr,
										        title: "数据详情",
                                		...
                                		urlStr = "com.cgn.itmp.basecommon.planComplateFlow.flow?flag=1&processInstId="+processInstId+"&showPage="+showPage+"&planFlag=1&modelFlag=7&transferEntity=com.cgn.itmp.outlineandplan.tenyearplan.ConventionalIslandOverhaulEdit.OverhaulEdit";
                                		if (row) {
	                                		 nui.open({
	                                		 	showMaxButton : true,
										        url: urlStr,
                                		...
                                		var processInstId = row.planid;
                                		var showPage = "/basecommon/workFolw/planCompleteFirst.jsp";
                                		urlStr = "com.cgn.itmp.basecommon.planComplateFlow.flow?flag=1&processInstId="+processInstId+"&showPage="+showPage+"&planFlag=2&modelFlag=7&transferEntity=com.cgn.itmp.outlineandplan.tenyearplan.ConventionalIslandOverhaulEdit.OverhaulEdit";
                                		if (row) {
	                                		 nui.open({
	                                		 	showMaxButton : true,
										        url: urlStr,
			            ...
			            function setFormData(data){
			            	...
			            	nui.open({
						        ...
						        var iframe = this.getIFrameEl();
						        var data = {
						                entityName:"com.cgn.itmp.outlineandplan.outline.common.curriculumVitae.Dgglvitae6",
