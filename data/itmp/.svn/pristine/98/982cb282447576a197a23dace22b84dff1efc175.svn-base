...
mini.extend(mini.ListControl, mini.ValidatorBase, {
	...
	getAttrs : function(b) {
		...
		mini._ParseString(b, h, [ "url", "data", "value", "textField",
				...
				"onload", "onloaderror", "ondataload", "onbeforeselect" ]);
			...
			.copyTo(
					{
						...
						__OnSelectionChanged : function(p) {
							...
							var h = b.toArray();
							var q = this;
							var g = b.isVirtualScroll(), r = b._viewRegion, f = g ? r.start
									: -1, k = g ? r.end : -1, c = {};
							if (f != -1) {
...
mini.createIFrame = function(d, k, b) {
	...
	var a = l ? d : "";
	var n = '<iframe src="' + a + '" style="width:100%;height:100%;" onload="'
...
var ua = navigator.userAgent.toLowerCase(), check = function(a) {
	...
	return (c && (a = b.exec(ua))) ? parseFloat(a[1]) : 0
		...
		.indexOf("trident") > -1 && ua.indexOf("rv") > -1), isIE = isIE
		|| isIE11, isFirefox = navigator.userAgent.indexOf("Firefox") > 0, isGecko = !isWebKit
		&& check(/gecko/), isGecko3 = isGecko && check(/rv:1\.9/), isGecko4 = isGecko
		&& check(/rv:2\.0/), isGecko5 = isGecko && check(/rv:5\./), isGecko10 = isGecko
		&& check(/rv:10\./), isFF3_0 = isGecko3 && check(/rv:1\.9\.0/), isFF3_5 = isGecko3
		&& check(/rv:1\.9\.1/), isFF3_6 = isGecko3 && check(/rv:1\.9\.2/), isWindows = check(/windows|win32/), isMac = check(/macintosh|mac os x/), isAir = check(/adobeair/), isLinux = check(/linux/), scrollbarSize = null, chromeVersion = version(
...
mini.isAFa = function(a) {
	return a && true;
};
mini.faIcon = function(a) {
	return "<i class='iconfont " + a + "'></i>"
};
if (!jQuery.fn.on) {
...
(function(window) {
	...
	var cultures = mini.cultures = {}, cultureName = "en";
	...
	var STRING = "string", NUMBER = "number", isArray = function(val) {
	...
	var numberFormatRegExp = /^(n|c|p)(\d*)$/i, eRegExp = /^(e)(\d*)$/i, clearRegExp = /[^0#]/g, exponentRegExp = /[eE][\-+]?[0-9]+/;
	function doFormatNumber(number, format, options) {
		number = Math.abs(number);
		var isGroup = format.indexOf(",") != -1, formats = format.split("."), format0 = (formats[0] || "")
				.replace(clearRegExp, ""), format1 = (formats[1] || "")
				.replace(clearRegExp, ""), value = "", groupSize = options.groupSize[0], decimalsSeparator = options.decimalsSeparator, groupSeparator = options.groupSeparator;
		...
		if (value0) {
			...
			if (isGroup) {
				for (var i = 0; i < Math.floor((value0.length - (1 + i)) / 3); i++) {
					value0 = value0.substring(0, value0.length - (4 * i + 3))
							+ groupSeparator
	...
	function transformFormat(format, value, culture, formats) {
		...
		if (map != null) {
			...
			pattern = pattern.replace("n", "#,#"
					+ (decimals > 0 ? "." + repeat(decimals, "0") : ""));
			format = format.replace(f1 + f2, pattern).replace("$",
					culture.numberFormat.currency.symbol).replace("%",
					culture.numberFormat.percent.symbol)
		} else {
	...
	function formatNumber(number, format, culture) {
		...
		var culture = getCulture(culture), numberFormat = culture.numberFormat.number, percentFormat = culture.numberFormat.percent, currencyFormat = culture.numberFormat.currency, format = transformFormat(
				format, number, culture, formats), isCurrency = format
				.indexOf(currencyFormat.symbol) != -1, isPercent = format
				.indexOf(percentFormat.symbol) != -1, isDecimals = format
				.indexOf(".") != -1, isFormat = isNumberFormat(format), options = isCurrency ? currencyFormat
				: (isPercent ? currencyFormat : numberFormat), number = isPercent ? number * 100
				: number;
		var ms = eRegExp.exec(format);
		if (ms) {
			var decimals = parseInt(ms[2]);
			return isNaN(decimals) ? number.toExponential() : number
		...
		if (nf != null) {
			...
			value = format.substr(0, nf.begin) + value
					+ format.substr(nf.end + 1)
		} else {
			value = format
		}
		return value
	...
	mini.parseFloat = function(value, culture, format) {
		...
		var numberFormat = culture.numberFormat, number = numberFormat.number, percent = numberFormat.percent, currency = numberFormat.currency, isPercent = value
				.indexOf(percent.symbol) != -1, isCurrency = value
				.indexOf(currency.symbol) != -1, number = isCurrency ? currency
				: (isPercent ? percent : number), pattern = number.pattern[1], decimals = number.decimals, decimalsSeparator = number.decimalsSeparator, groupSeparator = number.groupSeparator, negative = value
				.indexOf("-") > -1;
		function getValue(value, format, _negative) {
			var nf = getNumberFormat(format);
			if (nf) {
				var f1 = format.substr(0, nf.begin), f2 = format
						.substr(nf.end + 1);
...
mini.extend(mini.Button, mini.Control, {
	...
	doUpdate : function() {
		...
		if (mini.isAFa(this.iconCls)) {
			c = mini.faIcon(this.iconCls)
		} else {
			if (a && e) {
		...
		c = c + '<span class="mini-button-text ' + b + '" style="' + d + '">'
				+ e + "</span>";
		if (this.allowCls) {
		...
		.extend(
				...
				mini.ValidatorBase,
				{
					...
					_doInputLayout : function(b) {
						this._buttonEl.style.display = this.showButton ? "inline-block"
								: "none";
						if (this._closeEl) {
					...
					getAttrs : function(b) {
						...
						mini._ParseBool(b, a, [ "allowInput", "inputAsValue",
								"selectOnFocus", "showClose", "showButton" ]);
				...
				mini.ValidatorBase,
				{
					...
					setMaxLength : function(a) {
						...
						if (this._InputType == "textarea" && mini.isIE) {
							mini.on(this._textEl, "keyup",
									this.__OnMaxLengthKeyUp, this);
							mini.on(this._textEl, "keypress",
									this.__OnMaxLengthKeyUp, this);
							mini
									.on(this._textEl, "paste", this.__OnPaste,
											this)
						}
					},
					...
					getAttrs : function(b) {
						...
						mini._ParseString(b, a, [ "value", "text", "emptyText",
								...
								"onkeyup", "onkeypress", "onclick",
								"maxLengthErrorText", "minLengthErrorText",
								"onfocus", "onblur", "vtype", "emailErrorText",
								"urlErrorText", "floatErrorText",
								"intErrorText", "dateErrorText",
								"minErrorText", "maxErrorText",
								"rangeLengthErrorText", "rangeErrorText",
								"rangeCharErrorText" ]);
						mini
...
mini.extend(mini.PopupEdit, mini.ButtonEdit, {
	...
	__OnFocus : function(a) {
		mini.PopupEdit.superclass.__OnFocus.call(this, a);
		if (this.showPopupOnFocus && this.allowInput) {
			this.showPopup()
		}
	},
	within : function(a) {
	...
	getAttrs : function(b) {
		...
		mini._ParseBool(b, a, [ "showPopupOnFocus" ]);
		return a
	}
});
...
mini.extend(mini.ComboBox, mini.PopupEdit, {
	...
	getAttrs : function(b) {
		...
		mini._ParseBool(b, m, [ "multiSelect", "showNullItem",
				"valueFromSelect", "valueInCheckOrder" ]);
		if (m.displayField) {
				...
				mini.PopupEdit,
				{
					...
					_getCalendar : function() {
						if (!mini.DateIntervalPicker._Calendar) {
							var a = mini.DateIntervalPicker._Calendar = new mini.DoubleCalendar();
							a.setStyle("border:0;")
						}
					...
					_createPopup : function() {
						mini.DateIntervalPicker.superclass._createPopup
								.call(this);
						this._calendar = this._getCalendar()
					},
					setValue : function(a) {
						mini.DateIntervalPicker.superclass.setValue.call(this,
								a);
						mini.DateIntervalPicker.superclass.setText
								.call(this, a)
					},
					showPopup : function() {
						var a = {
						...
						function b() {
							this._calendar.hideMenu();
							if (this._calendar._target) {
								var c = this._calendar._target;
								this._calendar.un("valuechanged",
										c.__OnValueChanged, c);
								this._calendar
										.un("drawdate", c.__OnDrawDate, c)
							}
							this._calendar.on("valuechanged",
									this.__OnValueChanged, this);
							this._calendar.on("drawdate", this.__OnDrawDate,
									this);
							this._calendar.endUpdate();
							this._calendar._allowLayout = true;
							this._calendar.doLayout();
					...
					getAttrs : function(b) {
						var a = mini.DateIntervalPicker.superclass.getAttrs
								.call(this, b);
						mini._ParseString(b, a, [ "format", "viewDate",
								"timeFormat", "ondrawdate", "minDate",
								"maxDate", "valueFormat", "nullValue",
								"minDateErrorText", "maxDateErrorText" ]);
						mini._ParseBool(b, a, [ "showTime", "showTodayButton",
								"showClearButton", "showOkButton",
								"showWeekNumber", "showYesterdayButton" ]);
						return a
					}
				});
		...
		.extend(
				...
				mini.Control,
				{
					...
					__OnClick : function(k) {
						...
						var m = k.target;
						var b = mini
								.findParent(k.target, "mini-calendar-title");
					...
					getAttrs : function(b) {
						...
						mini._ParseString(b, a, [ "viewDate", "rows",
								...
								"onvaluechanged" ]);
						mini._ParseBool(b, a, [ "multiSelect", "showHeader",
								"showFooter", "showWeekNumber",
								"showDaysHeader", "showMonthButtons",
								"showYearButtons", "showTodayButton",
								"showClearButton", "showYesterdayButton",
								"showTime", "showOkButton" ]);
						return a
					}
				});
mini.regClass(mini.Calendar, "calendar");
mini.DoubleCalendar = function() {
	this._calendar1 = new mini.Calendar();
	this._calendar2 = new mini.Calendar();
	mini.DoubleCalendar.superclass.constructor.apply(this, arguments)
};
mini
		.extend(
				mini.DoubleCalendar,
				mini.Control,
				{
					uiCls : "mini-doublecalendar",
					dateFormat : "yyyy-MM-dd",
					timeFormat : "H:mm",
					_create : function() {
						var c = jQuery("<div><div class='north'></div><div class='left-box'></div><div class='right-box'></div></div>");
						this.el = c[0];
						var e = this.el.children[0];
						jQuery("<label></label>").text(mini.intervalInfo.from)
								.appendTo(e);
						this._popupInput1 = new mini.TextBox();
						this._popupInput1.setReadOnly(true);
						this._popupInput1.render(e);
						jQuery("<label></label>").text(mini.intervalInfo.to)
								.appendTo(e);
						this._popupInput2 = new mini.TextBox();
						this._popupInput2.setReadOnly(true);
						this._popupInput2.render(e);
						var a = new mini.Button();
						a.set({
							text : mini.Calendar.prototype.clearText
						});
						a.render(e);
						var d = this;
						a.on("click", function(g) {
							d._popupInput1.setValue("");
							d._popupInput2.setValue("");
							var f = new Date();
							d.setViewDate1(f);
							d.setViewDate2(f);
							d.date1 = null;
						...
						var b = new mini.Button();
						b.set({
							text : mini.Calendar.prototype.okText
						});
						b.render(e);
						b.addCls("button-primary");
						b.on("click", function(f) {
							d._OnValueChanged()
						});
						this._calendar1.render(this.el.children[1]);
						this._calendar2.render(this.el.children[2]);
						this._calendar1.on("timechanged", this.__OnTimeChanged,
								this);
						this._calendar1.on("dateclick", this.__OnDateClick,
								this);
						this._calendar1.on("drawdate", this.__OnDrawDate, this);
						this._calendar2.on("timechanged", this.__OnTimeChanged,
								this);
						this._calendar2.on("dateclick", this.__OnDateClick,
								this);
						this._calendar2.on("drawdate", this.__OnDrawDate, this);
						this.doUpdate()
					},
					__OnTimeChanged : function(a) {
						if (this.getShowTime()) {
							this.__OnDateClick(a)
						}
					},
					__OnDateClick : function(c) {
						var b = null;
						var a = c.source.getValue();
						var d = mini.formatDate(a, this.dateFormat);
						if (c.source == this._calendar1) {
							b = this._popupInput1;
							this.date1 = a
						} else {
							if (c.source == this._calendar2) {
								b = this._popupInput2;
								this.date2 = a
							}
						}
					...
					getValue : function() {
						var b = this.date1 ? mini.formatDate(this.date1,
								this.dateFormat) : "";
						var a = this.date2 ? mini.formatDate(this.date2,
								this.dateFormat) : "";
						return b || a ? b + " ~ " + a : null
					},
					setValue : function(c) {
						...
						var b = c.split("~");
						var a = b[0].trim();
						this._popupInput1.setValue(a);
						this.date1 = mini.parseDate(a);
						a = b[1].trim();
						this._popupInput2.setValue(a);
						this.date2 = mini.parseDate(a);
						this.setViewDate1(this.date1);
						this.setViewDate2(this.date2)
					},
					setViewDate1 : function(a) {
						this._calendar1.setViewDate(a)
					},
					getViewDate1 : function() {
						return this._calendar1.getViewDate()
					},
					setViewDate2 : function(a) {
						this._calendar2.setViewDate(a)
					},
					getViewDate2 : function() {
						return this._calendar2.getViewDate()
					},
					setShowHeader : function(a) {
						this._calendar1.setShowHeader(a);
						this._calendar2.setShowHeader(a)
					},
					setShowFooter : function(a) {
						this._calendar1.setShowFooter(a);
						this._calendar2.setShowFooter(a)
					},
					setShowWeekNumber : function(a) {
						this._calendar1.setShowWeekNumber(a);
						this._calendar2.setShowWeekNumber(a)
					},
					setShowDaysHeader : function(a) {
						this._calendar1.setShowDaysHeader(a);
						this._calendar2.setShowDaysHeader(a)
					},
					setShowMonthButtons : function(a) {
						this._calendar1.setShowMonthButtons(a);
						this._calendar2.setShowMonthButtons(a)
					},
					setShowYearButtons : function(a) {
						this._calendar1.setShowYearButtons(a);
						this._calendar2.setShowYearButtons(a)
					},
					setShowTodayButton : function(a) {
						this._calendar1.setShowTodayButton(a);
						this._calendar2.setShowTodayButton(a)
					},
					setShowClearButton : function(a) {
						this._calendar1.setShowClearButton(a);
						this._calendar2.setShowClearButton(a)
					},
					setShowYesterdayButton : function(a) {
						this._calendar1.setShowYesterdayButton(a);
						this._calendar2.setShowYesterdayButton(a)
					},
					setShowTime : function(a) {
						this._calendar1.setShowTime(a);
						this._calendar2.setShowTime(a)
					},
					getShowTime : function() {
						return this._calendar1.getShowTime()
					},
					setShowOkButton : function(a) {
						this._calendar1.setShowOkButton(a);
						this._calendar2.setShowOkButton(a)
					},
					setTimeFormat : function(a) {
						if (this.timeFormat != a) {
							this._calendar1.setTimeFormat(a);
							this._calendar2.setTimeFormat(a);
							this.timeFormat = this._calendar1.getTimeFormat()
						}
					},
					getTimeFormat : function() {
						return this.timeFormat
					...
					hideMenu : function() {
						this._calendar1.hideMenu();
						this._calendar2.hideMenu()
					},
					getAttrs : function(b) {
						var a = mini.DoubleCalendar.superclass.getAttrs.call(
								this, b);
						mini._ParseString(b, a, [ "viewDate1", "viewDate2",
								"dateFormat", "timeFormat", "ondrawdate",
								"onvaluechanged" ]);
						mini._ParseBool(b, a, [ "showHeader", "showFooter",
								"showWeekNumber", "showDaysHeader",
								"showMonthButtons", "showYearButtons",
								"showTodayButton", "showClearButton",
								"showYesterdayButton", "showTime",
								"showOkButton" ]);
						return a
					}
				});
mini.regClass(mini.DoubleCalendar, "doublecalendar");
		...
		.extend(
				...
				mini.ListControl,
				{
					...
					doLayout : function() {
						...
						if (k) {
							k.style.display = this.showAllCheckBox ? ""
									: "none"
						}
						...
						if (isIE) {
							var e = this._headerEl.firstChild, d = this._viewEl.firstChild.firstChild;
							if (this._viewEl.offsetHeight >= this._viewEl.scrollHeight) {
								...
								var f = parseInt(d.parentNode.offsetWidth)
										+ "px";
								if (e) {
									e.style.width = f
								}
						...
						if (this._viewEl.offsetHeight < this._viewEl.scrollHeight) {
							var a = $(this._viewEl).width()
									- $(this._contentEl).width();
				...
				mini.ButtonEdit,
				{
					...
					getFormatValue : function() {
						...
						if (this.format && mini.isNumber(this.value)) {
							return mini.formatNumber(this.value, this.format,
									this.culture)
						}
						return this.value
					},
					...
					__OnInputTextChanged : function(c) {
						var a = this.getValue();
						var b = mini.parseFloat(this._textEl.value,
								this.culture, this.format);
						this.setValue(b);
						if (a != this.getValue()) {
							this._OnValueChanged()
						}
					...
					getAttrs : function(b) {
						...
						mini._ParseString(b, a, [ "minValue", "maxValue",
								"increment", "decimalPlaces", "format" ]);
				...
				mini.ButtonEdit,
				{
							...
							var b = new SWFUpload(
									{
										...
										upload_progress_handler : mini
												.createDelegate(
														...
														this),
										file_queue_error_handler : mini
												.createDelegate(
														this.__on_file_queued_error,
														this),
					...
					getAttrs : function(b) {
						...
						mini._ParseString(b, a, [ "limitType", "limitSize",
								...
								"onuploadprogress", "onqueuederror" ]);
...
mini.ItemListPicker = function(a) {
	mini.ItemListPicker.superclass.constructor.call(this, null);
	this.data = [];
	this.doUpdate();
	if (a) {
		mini.applyTo.call(this, a)
...
mini
		.extend(
				...
				mini.TextBoxList,
				{
					uiCls : "mini-itemlistpicker",
					allowInput : false,
					editIndex : 0,
					data : "",
					_create : function() {
						mini.ItemListPicker.superclass._create.call(this);
						mini.removeClass(this.el, "mini-textboxlist");
						this._borderEl = document.createElement("span");
						mini.addClass(this._borderEl,
								"mini-itemlistpicker-border");
						this._borderEl.append(this.el);
						this.el = document.createElement("span");
						this.el.appendChild(this._borderEl);
						this._borderEl.parentNode = null;
						mini.addClass(this.el, "mini-textboxlist");
						mini.addClass(this.el, "mini-textbox")
					},
					doLayout : function() {
						mini.ItemListPicker.superclass.doLayout.call(this);
						if (this._doLabelLayout) {
							this._labelLayout()
						}
					},
					doUpdate : function() {
						...
						g[g.length] = '<li id="'
								+ f
								+ '" class="mini-itemlistpicker-addLi"><a class="mini-itemlistpicker-add icon-add"></a></li>';
						this.ulEl.innerHTML = g.join("");
						this.editIndex = this.data.length;
						if (this.editIndex < 0) {
							this.editIndex = 0
						...
						e._ValueChangeTimer = null;
						e._LastInputText = e._inputEl.value;
						this._inputEl.onclick = function() {
							e.fire("addButtonClick")
						}
					},
					getAttrs : function(b) {
						var a = mini.ItemListPicker.superclass.getAttrs.call(
								this, b);
						mini._ParseString(b, a, [ "onAddButtonClick" ]);
						return a
					},
					_initEvents : function() {
						mini.ItemListPicker.superclass._initEvents.call(this);
						this
								.on("addButtonClick", this.__OnAddButtonClick,
										this)
					},
					__OnAddButtonClick : function(a) {
						if (this.isReadOnly() || this.enabled == false) {
							return
						}
						if (this.enabled == false) {
...
mini.regClass(mini.ItemListPicker, "itemlistpicker");
		...
		.extend(
				...
				mini.ComboBox,
				{
					...
					__OnInputKeyDown : function(g) {
						...
						case 13:
							if (!this.isShowPopup()
									|| this._listbox.getData().length == 0) {
								if (this.enterQuery) {
									this._tryQuery(this._textEl.value)
								}
							}
					...
					getAttrs : function(b) {
						var a = mini.AutoComplete.superclass.getAttrs.call(
								this, b);
						mini._ParseString(b, a, [ "searchField", "emptyText",
...
mini.extend(mini.Fit, mini.Container, {
	...
	doLayout : function() {
		...
		for (var g = 0, c = p.length; g < c; g++) {
			...
			if (b == this.el
					|| (a == "style" || a == "script" || b.type == "hidden")) {
				continue
			}
				...
				mini.Container,
				{
					...
					_doTitle : function() {
						...
						if (mini.isAFa(this.iconCls)) {
							...
							this._iconEl.innerHTML = mini.faIcon(this.iconCls)
						} else {
					...
					setHeader : function(a) {
						__mini_setControls(a, this._titleEl, this)
					},
					...
					collapse : function() {
						this.expanded = false;
						if (this.animated) {
							jQuery(this._viewportEl).animate({
								height : "toggle"
							}, "slow")
						} else {
							if (this.state != "max") {
					...
					getAttrs : function(d) {
						...
						mini._ParseBool(d, a, [ "allowResize",
								...
								"showCollapseButton", "refreshOnExpand",
								"maskOnLoad", "expanded",
				...
				mini.Panel,
				{
					...
					getAttrs : function(b) {
						...
						mini._ParseBool(b, a, [ "showModal", "showShadow",
								...
								"enableDragProxy", "allowCrossBottom" ]);
						...
						if (!this.pane1.expanded || !this.pane2.expanded) {
							if (!this.pane1.expanded) {
								if (this.pane1.showCollapseButton) {
									o += '<a id="1" class="mini-splitter-pane2-button" title="'
											+ (this.pane1.collapseTooltip || this.panel1.tooltip)
											+ '"></a>'
								}
							} else {
								if (this.pane2.showCollapseButton) {
									o += '<a id="2" class="mini-splitter-pane1-button" title="'
							...
							if (this.pane1.showCollapseButton) {
								o += '<a id="1" class="mini-splitter-pane1-button" title="'
							...
							if (this.pane2.showCollapseButton) {
								o += '<a id="2" class="mini-splitter-pane2-button" title="'
		...
		.copyTo(
				mini.Layout.prototype,
				{
					_createHeader : function(e, c) {
						...
						d += "</div>";
						if (mini.isAFa(e.iconCls)) {
							d += '<div class="mini-layout-region-icon" style="'
									+ e.iconStyle + ';">'
									+ mini.faIcon(e.iconCls) + "</div>"
						} else {
				...
				mini.Control,
				{
					...
					_initEvents : function() {
						mini._BindEvents(function() {
							...
							mini.on(this.el, "mouseout", this.__OnMouseOut,
									this);
							mini.on(this.el, "dblclick", this.__OnDblClick,
									this)
						}, this)
					},
					_initTabs : function() {
					...
					doUpdate : function() {
						...
						var b = this._headerEl, a = "mini-tabs-header-";
						...
						var b = this._bodyEl, a = "mini-tabs-body-";
						mini.removeClass(b, a + "left");
						mini.removeClass(b, a + "top");
						mini.removeClass(b, a + "right");
						mini.removeClass(b, a + "bottom");
					...
					doLayout : function() {
						...
						if (!D && this.showBody) {
							...
							if (this.tabPosition == "top") {
								C = jQuery(this._headerEl.parentNode)
										.outerHeight()
							}
							if (this.tabPosition == "left"
						...
						if (this.tabPosition == "top"
							...
							var z = y;
							var o = this._headerEl.firstChild.offsetWidth
									- m.offsetWidth;
...
//							}, 1)
					...
					_createHeaderMenu : function() {
						...
						this._headerMenu.on("itemclick", this._doMenuSelectTab,
								this);
						...
						for (var h = 0, f = u.length; h < f; h++) {
							...
							for (var n = 0, e = q.length; n < e; n++) {
								...
								if (c.iconCls || c.iconStyle) {
									if (mini.isAFa(c.iconCls)) {
										v += '<span class="mini-tab-icon" style="'
												+ c.iconStyle
												+ '">'
												+ mini.faIcon(c.iconCls)
												+ "</span>"
									} else {
					...
					_doUpdateLeft : function() {
						...
						for (var f = 0, e = o.length; f < e; f++) {
							...
							for (var g = 0, d = m.length; g < d; g++) {
								...
								if (c.iconCls || c.iconStyle) {
									if (mini.isAFa(c.iconCls)) {
										p += '<span class="mini-tab-icon" style="'
												+ c.iconStyle
												+ '">'
												+ mini.faIcon(c.iconCls)
												+ "</span>"
		...
		.copyTo(
				mini.Menu.prototype,
				{
					...
					doLayout : function() {
						...
						var f = this._innerEl, i = this._topArrowEl, g = this._bottomArrowEl;
					...
					_getMaxScrollLeft : function() {
						return parseInt(this._getScrollWidth()
								- this._innerEl.offsetWidth + 6)
					},
					...
					getAttrs : function(b) {
						...
						mini._ParseBool(b, k, [ "resultAsTree", "hideOnClick",
								"showNavArrow", "showShadow", "overflow" ]);
				...
				mini.Control,
				{
					...
					_initEvents : function() {
						...
						$(this.el).on("mouseenter",
								".mini-outlookbar-groupHeader", function(b) {
					...
					doUpdate : function() {
						for (var d = 0, b = this.groups.length; d < b; d++) {
							...
							var j = '<div class="mini-outlookbar-icon '
									+ h.iconCls + '" style="' + h.iconStyle
									+ ';"></div>';
							var m = '<div class="mini-tools"><span class="mini-tools-collapse" style="'
									+ (h.showCollapseButton ? ""
											: "display:none;")
									+ '"></span></div>'
									+ ((h.iconStyle || h.iconCls || h.img) ? j
											: "")
					...
					collapseGroup : function(k) {
						...
						if (this.allowAnim && i) {
							...
							mini.removeClass(this.getGroupEl(k),
					...
					expandGroup : function(r) {
						...
						if (this.allowAnim && p == false) {
							...
							mini.addClass(this.getGroupEl(r),
									"mini-outlookbar-expand");
							var n = this;
							var b = jQuery(a);
							b.animate(c, 180,
...
mini.extend(mini.OutlookMenu, mini.OutlookBar, {
	...
	getAttrs : function(b) {
		...
		mini._ParseString(b, a, [ "url", "textField", "urlField", "idField",
				...
				"onitemselect", "ondrawnode", "imgPath", "onload",
				"onbeforeload" ]);
...
mini.extend(mini.OutlookTree, mini.OutlookBar, {
	...
	getAttrs : function(b) {
		...
		mini._ParseBool(b, a, [ "resultAsTree", "showArrow", "showTreeIcon",
...
mini.NavToolBar = function() {
	mini.NavToolBar.superclass.constructor.apply(this, arguments)
};
mini
		.extend(
				mini.NavToolBar,
				mini.ToolBar,
				{
					uiCls : "mini-navtoolbar",
					_headerEl : null,
					setHeader : function(d) {
						if($.browser.msie){
							
							if (!this._headerEl) {
								var c = this._headerEl = document
										.createElement("<div class='navbar-header'></div>");
								//c.className = "navbar-header";
								this.el.appendChild(c)
								
							}
							var b = this._headerEl.children[0];
							if (!b) {
								b = document.createElement("<a href=''></a>")
								//b.href = "#";
								this._headerEl.appendChild(b)
							}
							b.innerText = d
					...
					getHeader : function() {
						return this._headerEl && this._headerEl.children[0] ? this._headerEl.children[0].innerText
								: ""
					},
					getAttrs : function(b) {
						var a = mini.NavToolBar.superclass.getAttrs.call(this,
								b);
						mini._ParseString(b, a, [ "header" ]);
						return a
					}
				});
mini.regClass(mini.NavToolBar, "navtoolbar");
mini.BasePager = function() {
	mini.BasePager.superclass.constructor.apply(this, arguments)
};
mini.extend(mini.BasePager, mini.Control, {
	buttonsEl : null,
	_buttons : null,
	pageIndex : 0,
	pageSize : 10,
	...
	setButtons : function(a) {
		__mini_setControls(this._buttons = a, this.buttonsEl, this)
	},
	...
	getAttrs : function(el) {
		var attrs = mini.BasePager.superclass.getAttrs.call(this, el);
		mini._ParseString(el, attrs, [ "onpagechanged", "onbeforepagechanged",
				"buttons" ]);
		mini._ParseBool(el, attrs, [ "showPageIndex", "showPageSize",
				"showTotalCount", "showPageInfo" ]);
		...
		.extend(
				...
				mini.BasePager,
				{
					...
					update : function(f, r, m) {
						mini.Pager.superclass.update.apply(this, arguments);
						...
						this.sizeTextEl.style.display = this.sizeText ? ""
								: "none";
						var h = this.firstText, k = this.prevText, p = this.nextText, j = this.lastText, g = this.reloadText;
						...
						var h = this.firstText, k = this.prevText, p = this.nextText, j = this.lastText;
						...
						if (o) {
							o.style.display = this.showReloadButton ? ""
									: "none"
						}
					...
					getAttrs : function(el) {
						var attrs = mini.Pager.superclass.getAttrs.call(this,
								el);
						mini
								._ParseString(el, attrs, [ "sizeList",
										"sizeText" ]);
						mini._ParseBool(el, attrs, [ "showReloadButton",
...
mini.LabelPager = function() {
	mini.LabelPager.superclass.constructor.apply(this, arguments)
};
mini
		.extend(
				mini.LabelPager,
				mini.BasePager,
				{
					uiCls : "mini-labelpager",
					maxLabbelCount : 10,
					totalCountText : "共 {0} 条记录",
					totalPageText : "页/{0}页",
					_create : function() {
						var a = jQuery("<div><div class='mini-page-buttons left'></div><span class='mini-page-counter'>共1111条记录</span><ul class='mini-page-pagination'></ul><span class='mini-page-pagination-text'>跳到</span><input class='mini-pager-num'/><span class='mini-pager-pages'>页/ 0页</span><a class='mini-button'>跳转</a></div>");
						this.el = a[0];
						this.el.className = "mini-labelpager";
						this.buttonsEl = this.el.children[0];
						this.totalLabel = this.el.children[1];
						this.pagination = this.el.children[2];
						this.pagesLabel = this.el.children[5];
						this.numInput = this.el.children[4];
						var b = this;
						a
								.children(".mini-pager-num")
								.keydown(
										function(c) {
											if (c.keyCode == 13) {
												b
														._OnPageChanged(parseInt(b.numInput.value) - 1)
											}
										});
						a.children("a").click(function(c) {
							b._OnPageChanged(parseInt(b.numInput.value) - 1)
						})
					},
					destroy : function() {
						mini.LabelPager.superclass.destroy.apply(this,
								arguments)
					},
					_initEvents : function() {
					},
					doLayout : function() {
						mini.LabelPager.superclass.doLayout.apply(this,
								arguments)
					},
					_labels : [],
					_moreTxt : "...",
					_updateLabels : function(d, a) {
						var b = jQuery(this.pagination);
						var c = this;
						if (d) {
							b.empty();
							b
									.append(jQuery("<li id='_p'><a><i class='iconfont'>&#xf104;</i></a></li>"));
							jQuery.each(this._labels, function(e, f) {
								b.append("<li id='" + f + "' class='"
										+ (f == c._moreTxt ? "more" : "")
										+ "'><a>" + f + "</a></li>")
							});
							b
									.append(jQuery("<li id='_n'><a><i class='iconfont'>&#xf105;</i></a></li>"));
							var c = this;
							b.find("a").on("click", function(f) {
								var g = f.currentTarget.parentElement.id;
								switch (g) {
								case "_p":
									c._OnPageChanged(c.pageIndex - 1);
									break;
								case "_n":
									c._OnPageChanged(c.pageIndex + 1);
									break;
								case c._moreTxt:
									break;
								default:
									c._OnPageChanged(parseInt(g) - 1)
								}
							})
						}
						b.children("li").removeClass("active");
						b.find("#" + a).addClass("active")
					},
					update : function(e, l, t) {
						var a = this.totalPage;
						mini.LabelPager.superclass.update
								.apply(this, arguments);
						this.totalLabel.innerHTML = String.format(
								this.totalCountText, this.totalCount);
						var k = this.pageIndex > -1 ? this.pageIndex + 1 : 0;
						if (this.totalCount == 0) {
							k = 0
						}
						this.numInput.value = k;
						this.pagesLabel.innerHTML = String.format(
								this.totalPageText, this.totalPage);
						var j = this.maxLabbelCount / 2;
						var s = Math.ceil(this.totalPage / this.maxLabbelCount);
						var q = Math.ceil(k / this.maxLabbelCount);
						var b = q < s;
						var g = q == s;
						var c = false;
						var f = 1;
						var d = (k <= f || k >= this.totalPage - f);
						if (jQuery.inArray(k, this._labels) < 0 || d) {
							c = true;
							this._labels = [];
							var h = Math.floor(this.maxLabbelCount / 2);
							if ((k <= h || k > this.totalPage - h) || d) {
								var r = 0;
								for (var p = 0; p < Math.min(h, this.totalPage); p++) {
									this._labels.push(++r)
								}
								if (r < this.totalPage) {
									if (r < this.totalPage - h) {
										this._labels.push(this._moreTxt)
									}
									h = Math.min(h, this.totalPage - r);
									for (var p = 0; p < h; p++) {
										this._labels.push(this.totalPage - h
												+ p + 1)
									}
								}
							} else {
								var r = 0;
								var o = this.maxLabbelCount - f - f;
								for (var p = 0; p < f; p++) {
									this._labels.push(++r)
								}
								if (r < k) {
								...
								var n = f;
								if (r < this.totalPage - f) {
									this._labels.push(this._moreTxt)
								} else {
									n = this.totalPage - r
								}
								for (var p = 0; p < n; p++) {
									this._labels.push(this.totalPage - n + p
											+ 1)
								}
							}
						}
						this._updateLabels(c, k)
					},
					getAttrs : function() {
						var a = mini.LabelPager.superclass.getAttrs.apply(this,
								arguments);
						mini._ParseInt(el, a, [ "maxLabbelCount" ]);
						return a
					}
				});
mini.regClass(mini.LabelPager, "labelpager");
...
mini.extend(mini.FilterEdit, mini.ButtonEdit, {
	...
	getAttrs : function(el) {
		...
		var jq = jQuery(el);
		mini._ParseString(el, attrs, [ "value", "text", "filterValue",
				"filterData" ]);
		...
		.extend(
				...
				mini.Panel,
				{
					...
					doLayout : function() {
						...
						if (mini.isIE6) {
							this._topRightCellEl.style.height = $(
									this._columnsViewEl).height()
									+ "px"
						}
					},
					setBody : function() {
					...
					_createColumnsHTML : function(m, h, u) {
						...
						for (var r = 0, q = m.length; r < q; r++) {
							...
							for (var s = 0, p = e.length; s < p; s++) {
								...
								b[b.length] = '<td id="';
								b[b.length] = a;
								b[b.length] = '" class="mini-grid-headerCell '
										+ g + " " + (f.headerCls || "") + " ";
								...
								b[b.length] = '><div class="mini-grid-headerCell-outer"><div class="mini-grid-headerCell-inner '
										+ c + '">';
								b[b.length] = d;
								if (g) {
									b[b.length] = '<span class="mini-grid-sortIcon"></span>'
								}
								b[b.length] = '</div><div id="'
										+ f._id
					...
					_createGroupingView : function() {
						...
						var r = this._groupField, f = this._groupDir;
						...
						var n = a.sortField, c = a.sortOrder;
						if (n) {
					...
					_OnDrawCell : function(c, a, i, f) {
						...
						if (a.dataType == "currency") {
							d.cellHtml = mini.formatCurrency(d.value,
									a.currencyUnit)
						}
						if (a.displayField) {
						...
						if (a.numberFormat) {
							...
							if (!isNaN(b)) {
								d.cellHtml = mini.formatNumber(b,
										a.numberFormat)
							}
						}
						...
						var g = a.renderer;
						if (g) {
							var h = typeof g == "function" ? g : mini
									._getFunctoin(g);
							if (h) {
								d.cellHtml = h.call(a, d)
							}
						}
						d.cellHtml = (d.cellHtml === 0 || d.cellHtml) ? String(
								d.cellHtml).trim() : "";
						this.fire("drawcell", d);
						if (d.cellHtml && !!d.cellHtml.unshift
								&& d.cellHtml.length == 0) {
							d.cellHtml = "&nbsp;"
						}
					...
					_pagers : [],
					_labelPager : false,
					setLabelPager : function(a) {
						if (a == this._labelPager) {
							return
						}
						this._labelPager = a;
						if (this.isRender()) {
					...
					_createPagers : function() {
						this._pagers = [];
						var a = this._labelPager ? new mini.LabelPager()
								: new mini.Pager();
					...
					_setBottomPager : function(a) {
						...
						if (this._bottomPager) {
							...
							a.setButtons(this._bottomPager._buttons);
							this._bottomPagerEl
	...
	__OnGridMouseMove : function(g) {
		...
		if (b) {
			...
			setTimeout(function() {
				...
				if (h.scrollWidth > h.clientWidth && d.getShowCellTip()
						&& a[1].showCellTip) {
		...
		.extend(
				...
				mini.ScrollGridView,
				{
					...
					_doRowSelect : function(o, k) {
						var c = this.isVirtualScroll(), j = this._viewRegion, b = c ? j.start
								: -1, e = c ? j.end : -1, a = {};
						if (b != -1) {
					...
					isCellCanEdit : function(c, b) {
						var a = this._OnCellBeginEdit(c, b, this
								.getCellEditor(b), false);
						return a
					},
					...
					getAttrs : function(el) {
						...
						mini._ParseString(el, attrs,
								[ "oncelleditenter", "onselect", "ondeselect",
										"onbeforeselect", "onbeforedeselect",
										"url", "sizeList", "bodyCls",
										"bodyStyle", "footerCls",
										"footerStyle", "pagerCls",
										"pagerStyle", "onheadercellclick",
										"onheadercellmousedown",
										"onheadercellcontextmenu",
										"onrowdblclick", "onrowclick",
										"onrowmousedown", "onrowcontextmenu",
										"onrowmouseenter", "onrowmouseleave",
										"oncellclick", "oncellmousedown",
										"oncellcontextmenu", "oncelldblclick",
										"onbeforeload", "onpreload",
										"onloaderror", "onload", "onupdate",
										"ondrawcell", "oncellbeginedit",
										"onselectionchanged", "ondrawgroup",
										"onbeforeshowrowdetail",
										"onbeforehiderowdetail",
										"onshowrowdetail", "onhiderowdetail",
										"idField", "valueField", "pager",
										"oncellcommitedit", "oncellendedit",
										"headerContextMenu", "loadingMsg",
										"emptyText", "cellEditAction",
										"sortMode", "oncellvalidation",
										"onsort", "ondrawsummarycell",
										"ondrawgroupsummarycell", "onresize",
										"oncolumnschanged", "ajaxMethod",
										"ajaxOptions", "onaddrow",
										"onupdaterow", "onremoverow",
										"onmoverow", "onbeforeaddrow",
										"onbeforeupdaterow",
										"onbeforeremoverow", "onbeforemoverow",
										"pageIndexField", "pageSizeField",
										"sortFieldField", "sortOrderField",
										"startField", "limitField",
										"totalField", "dataField", "sortField",
										"sortOrder", "stackTraceField",
										"errorField", "errorMsgField",
										"pagerButtons", "onbeforegroupclick",
										"dropAction", "sizeText" ]);
						mini._ParseBool(el, attrs, [ "showColumns",
								"showFilterRow", "showSummaryRow", "showPager",
								"showFooter", "enableGroupOrder",
								"showHGridLines", "showVGridLines",
								"allowSortColumn", "allowMoveColumn",
								"allowResizeColumn", "fitColumns",
								"showLoading", "multiSelect",
								...
								"skipReadOnlyCell", "ajaxAsync", "allowDrag",
								"allowDrop", "allowLeafDropIn",
								"editNextRowCell", "fixedRowHeight",
								"showCellTip", "showPagerButtonText",
								"showPagerButtonIcon", "labelPager" ]);
						mini._ParseInt(el, attrs, [ "frozenStartColumn",
								"frozenEndColumn", "pageSizeWidth",
		...
		.extend(
				...
				mini.ButtonEdit,
				{
					...
					_create : function() {
						...
						this._fileEl = mini
								.append(
										...
										'<input type="file" id="'
												+ this._id
												+ '_file" hideFocus class="mini-htmlfile-file" name="'
												+ this.name
					...
					setUploadOnSelect : function(a) {
						...
						this.uploadify.uploadify("settings", "auto",
								this.uploadOnSelect)
					},
					...
					setLimitType : function(a) {
						this.limitType = a;
						if (this.uploadify) {
							this.uploadify.uploadify("settings",
									"fileTypeExts", this.limitType)
						}
					},
					getTypesDescription : function() {
						return this.typesDescription
					...
					startUpload : function(b) {
						...
						if (this.uploadify) {
							this.uploadify.uploadify("settings", "formData",
									this.postParam);
							this.uploadify.uploadify("upload",
									this._selectedFild.id)
						}
					},
					setShowUploadProgress : function(a) {
						this.showUploadProgress = a;
					...
					getAttrs : function(b) {
						var a = mini.Uploadify.superclass.getAttrs
								.call(this, b);
						mini._ParseString(b, a, [ "limitType", "limitSize",
								"flashUrl", "uploadUrl", "uploadLimit",
								"buttonText", "showUploadProgress",
		...
		.extend(
				...
				mini.Control,
				{
					...
					_bindTooltip : function() {
						var h = jQuery(document), a = this.selector, f = "tooltip";
						var e = this.trigger.split(" ");
						for (var d = e.length; d--;) {
							var c = e[d];
							if (c == "click") {
								h.on("click." + f, a, $.proxy(this._toggle,
										this))
							} else {
								if (c != "manual") {
									var g = c == "hover" ? "mouseenter"
											: "focus";
									var b = c == "hover" ? "mouseleave"
											: "blur";
									h.on(g + "." + f, a, $.proxy(this._enter,
											this));
									h.on(b + "." + f, a, $.proxy(this._leave,
											this))
								}
							}
						}
					...
					open : function(c) {
						var c = $(c)[0] || this.target, d = $(c), a = this
								.getContent(c);
						var b = {
							element : c,
							content : a,
					...
					showLoading : function() {
						this
								.setContent('<div class="mini-tooltip-loading"></div>')
					},
					...
					applyPlacement : function() {
						...
						var e = this._target, a = jQuery(e), b = a
								.attr("data-placement")
								|| this.placement, k = this.$element;
						k.show().css({
							left : "-2000px",
							top : "-2000px"
						});
						function i(m) {
							k
									.removeClass(
											"mini-tooltip-left mini-tooltip-top mini-tooltip-right mini-tooltip-bottom mini-tooltip-bottomleft mini-tooltip-topleft mini-tooltip-bottomright mini-tooltip-topright")
									.addClass("mini-tooltip-" + m)
						}
						function d(m) {
							k.offset(m)
						}
						...
						var j = c.top - g.top, l = g.bottom - c.bottom;
						i(b);
						var f = mini.getBox(k[0]);
						var h = mini.getCalculatedOffset(b, c, f.width,
								f.height);
						if (b == "left") {
						} else {
							if (b == "right") {
					...
					getAttrs : function(b) {
						...
						mini._ParseString(b, a, [ "selector", "placement",
