...
<!-- 		<input id='sortfield' class="nui-hidden" name="defectsshowdetailedinformation.sortfield"/> -->
		...
		<table style="width: 100%; height: 100%; table-layout: fixed;"
			...
			<tr>
				...
				<td colspan="1"><input class="nui-dictcombobox" dictTypeId="YJJCFF" emptyText="请选择检查方法"
					name="defectsshowdetailedinformation.inspectionmethod" emptyText="请输入检查方法" requiredErrorText="检查方法不能为空" required="required" style="width: 100%" /></td>
			...
			<tr>
				...
				<input class="nui-hidden" id="defectnum"
					...
					<td class="form_label">螺栓标识号</td>
				<td colspan="1"><input class="nui-textbox"
					name="defectsshowdetailedinformation.boltidentificationnumber" emptyText="请输入螺栓标识号"  requiredErrorText="螺栓标识号不能为空" required="required" style="width: 100%" /></td>
			</tr>
			<tr>
				<td class="form_label">缺陷类型</td>
				<td colspan="1"><input class="nui-textbox"
					name="defectsshowdetailedinformation.defecttype" emptyText="请输入缺陷类型" requiredErrorText="缺陷类型不能为空" required="required" style="width: 100%" /></td>
			...
			<tr>
				<td class="form_label">Hd-Hr(dB)</td>
				<td colspan="1"><input class="nui-textbox"
					name="defectsshowdetailedinformation.hdhr" style="width: 100%" /></td>
				<td class="form_label">深度位置</td>
				<td colspan="1"><input class="nui-textbox"
					name="defectsshowdetailedinformation.depthposition" emptyText="请输入深度位置" requiredErrorText="深度位置不能为空" required="required" style="width: 100%" /></td>
			</tr>
			<tr>
			<td class="form_label">周向位置</td>
				<td colspan="1"><input class="nui-textbox"
					name="defectsshowdetailedinformation.circumferentialposition" emptyText="请输入周向位置" requiredErrorText="周向位置不能为空" required="required" style="width: 100%" /></td>
	...
	<script type="text/javascript">
					...
					url:"/itmp/inspectcomponentmanage/defectdisplaymanagement/defectdisplaydetails/HistoryNumCheck.jsp",
					  title: "历史编号选择", width: 516, height:300,
					   ...
					   var iframe = this.getIFrameEl();
		                    var data = {stencilplate:"lssdcs"};//传入页面的json数据
		...
		function saveData() {
			...
			data.defectsshowdetailedinformation.stencilplate="lssdcs";
...
// 				nui.get('sortfield').setValue(json.defectsshowdetailedinformation.sortfield);
			...
			//如果是点击编辑类型页面
			if (infos.pageType == "edit" || infos.pageType == "lssdcs") {
