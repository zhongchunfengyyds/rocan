...
<script type="text/javascript">
    ...
    function saveData() {
        ...
        var success = true;
        for (var i = 0; i < json.tcgncgdylgd.length; i++) {
            var jsondata = "{'tcgncgdylgd':{'parms':'remove','uuid':'" + json.tcgncgdylgd[i].uuid + "','isdelete':'1','cname':'" + json.tcgncgdylgd[i].checkprojectname + "','currentuser':'" + $('#creator').val() + "','currentdatetime':'" + $('#datetime').val() + "','currentreason':'" + $('#reason').val() + "'}}";
            $.ajax({
                url: "com.cgn.itmp.outlineandplan.specialinspection.ConventionalIslandOverhaul.LJCGDYLGD.biz.ext",
                type: 'POST',
                data: jsondata,
                cache: false,
