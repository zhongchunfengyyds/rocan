		...
		<div class="search-container folding-menu" id="queryForm">
			<div style="width: 100%; height: 100%;">
				<table cellpadding="0" cellspacing="0" class="table Keywordsearchtable" style="height: 100%;">
					<tr class="">
						...
						<td class="form_label" style="width: 40%;">
							...
							显示周期---
								年度<input class="nui-spinner" id="start" maxValue="2089" value="1991" minValue="1991" style="width: 55px" name="" />-年度<input
				...
				<div id="overhaulform" class="nui-form display" align="center" style="height: 16%">
					<!-- 数据实体的名称 -->
					<input class="nui-hidden" name="criteria/_entity" value="com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandDaily.DailySearch">
					<!-- 排序字段 -->
					...
					<table id="table1" class="table" style="height: 100%">
						...
						<tr>
							...
							<td colspan="1" style="display: none">
								...
								<input id="detailed1" class="nui-textbox" name="criteria/_expr[8]/detailed" />
	            ...
	            <!-- <div class="form-group"> -->
	              <div>
                    ...
                    <tr>
                        ...
                        <td colspan="1">
                            <input class="nui-textbox" name="criteria/_expr[2]/site"/>
...
<!--                             <a  class="nui-menubutton btn btn-default btn-xs" menu="#popupMenu" id="planupdate"> -->
<!--                                <i class="fa fa-refresh"></i>计划更新 -->
<!--                             </a> -->
      ...
      <div class="nui-layout" style="width:100%;height:100%;" > 
    	...
    	<div title="center" region="center" >
            ...
            <div class="nui-fit">
                <div 
                        id="datagrid1"
                        dataField="tdgglsnjhhdrcs"
                        class="nui-datagrid"
                        style="width:100%;height:100%;"
                        url="com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandDaily.queryTdgglsnjhhdrcs.biz.ext"
                    ...
                    <div property="columns">
                        ...
                        <div field="datetime" visible="false" headerAlign="center" allowSort="true" Align="center" dateFormat="yyyy-MM-dd HH:mm:ss">
                            日期
                        </div>
                        <div field="base" headerAlign="center" visible="false" allowSort="true" Align="center" visible="false">
                            基地
                        </div>
                        <div field="crew" headerAlign="center" visible="false" allowSort="true" Align="center" visible="false">
                            机组
                        </div>
                        <div field="protype" headerAlign="center" visible="false" allowSort="true" Align="center" visible="false">
                            项目类型
                        </div>
                        <div id="parts" name="parts" field="parts" visible="false" headerAlign="center" Align="center" allowSort="true" >
                           设备/系统
                        </div>
                        ...
                        <div field="detailed" renderer="goon" headerAlign="center" allowSort="true" Align="center">
                            维护项目
                        </div>
	...
	$.ajax({
	    ...
	    success:function(data){
	    	...
	    	if(returnJson.data.sucess){
	            ...
	            nui.get('tree1').setUrl("com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandDaily.getProtypeAndPartsData.biz.ext?baseid="+jdid+"&crewid="+jzid);
// 	            search();
...
tree.on("nodeselect", function (e) {
    if (e.isLeaf) {
       //grid.load({"criteria/_entity": "com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandDaily.DailyEntity","criteria/_expr[4]/parts" : e.node.name });
       ...
       if('规范法定项目' == e.node.name){
    		parms = 'GFFD';
    	}else if('规范非法定项目' == e.node.name){
    		parms = 'GFFFD';
    	}
    	parts="";
		parentid = parms;
       //grid.load({"criteria/_entity": "com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandOverhaul.OverhaulEntity","criteria/_expr[5]/protype" : parms });
...
function add() {
	...
	if (row) {
		nui.open({showMaxButton : true,
	        url: "/itmp/tenyearplan/NuclearIsland/Daily/addForm.jsp",
	        title: "新增记录", width: 800, height: 400,
	        onload: function () {//弹出页面加载完成
		        var iframe = this.getIFrameEl();
		        var data = {pageType:"add",record:{tdgglsnjhhdrc:row},parts:row.parts,protype:row.protype};//传入页面的json数据
...
function edit() {
    ...
    if (row) {
        var json = {};
    	json.datas = row;
    	json.entityName = "com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandOverhaulEdit.NuclearIslandEdit";
    	json.fieldName = "uuid";
    	...
    	$.ajax({
                ...
                success:function(data){
                	...
                	if(returnJson.record != ''){
                		...
                		nui.open({showMaxButton : true,
			            url: "/itmp/tenyearplan/NuclearIsland/Daily/editForm.jsp",
			            title: "编辑数据",
			            width: 800,
			            height: 400,
			            onload: function () {
...
function openrecord(){
	nui.open({showMaxButton : true,
            url: "/itmp/tenyearplan/NuclearIsland/Daily/vitaeList.jsp",
            title: "变更履历",
            width: 1000,
            height: 520,
            onload: function () {
...
function synchronous(){
	var row = grid.getSelected();
    if (row) {
    	var jsondata = '{"tdgglsnjhhdrc":'+ nui.encode(row)+'}';
    	var urlStr = "com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandDaily.synchronous.biz.ext";
		$.ajax({
            url:urlStr,
            type:'POST',
            data:jsondata,
    	...
    	$.ajax({
		  ...
		  nui.confirm("确定删除选中记录？","系统提示",
		            ...
		            onload: function () {
			                ...
			                ,'processInstName':'十年计划(核岛)_日常执行条目_删除','processInstDesc':'十年计划(核岛)_日常执行条目_删除','entityType':'com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandOverhaulEdit.NuclearIslandEdit','modelFlag':6}};
			                //直接从页面获取，不用去后台获取
			                iframe.contentWindow.setFormData(data);
            },
            ondestroy: function (action) {
...
function recover(){
	nui.open({showMaxButton : true,
            url: "/itmp/tenyearplan/NuclearIsland/Daily/recycleList.jsp",
            title: "回收站查询",
            width: 800,
            height: 400,
            onload: function () {
	   		 ...
	   		 var count = data.count;
	   			if(count > 0){
	   				nui.alert("该基地升版流程正在审批中,不能升版","提示");
						...
						nui.open({
							showMaxButton : true,
				            url: "/itmp/outlineCommon/dgEdition/edtionStart.jsp?modelFlag=6&entity=com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandDaily.DailyEntity&basename=base&crewname=crew&editionname=edition",
				            title: "升版",
				            width: 500,
				            height: 330,
				            onload: function () {
...
function selectionChanged(){
 ...
 function output(){
    var form = new nui.Form("#overhaulform");
    var json = form.getData(false,false);
    var filterField = "processstatus,datetime,uuid,orderno,processinstid,creator,reason,isdelete";
	...
	var defaultCheck="detailed,inspectionscope,site";
    nui.open({showMaxButton : true,
        ...
        var iframe = this.getIFrameEl();
        var data = {
                entityName:"com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandDaily.DailySearch",
                queryParam:json,								    //查询数据
                queryKey:'criteria',                              //查询实体名
                query:['itemno','site','detailed','parts','protype','base','crew'],  //按顺序放置
                dicFieds: "protype@PROTYPE",
                expTitle:"十年计划核岛日常执行条目",
...
function batchAdd(){
	...
	$.ajax({
        ...
        onload: function () {//弹出页面加载完成
            ...
            var data = {
            	pageType:"batchAdd",
            	tempFileName:"snhdrczxtm.xlsx",
            	tempLoadName:"十年计划核岛部件日常执行条目导入模板",
            	entitfName:"com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandDaily.DailyEntity"
            };//传入页面的json数据
	...
	function onCloseClick(e) {
		...
		function btnQuery() {
			...
			var data = {
				matchFied : "itemno,inspectionscope,detailed,edition,remark,site,period",
				wordVal : wordVal,
		...
		function goon(e){
			if(e.record.detailed != null){
				return '<a href = "#" onclick ="showdetail(\''+e.record.detailed+'\')">'+e.record.detailed+'</a>';
		 	}
		}		
		
		function showdetail(detailed){
			var json ={};
			json.fieldName = "detailed";
			json.fieldValue = detailed;
			json.entityOne = "com.cgn.itmp.outlineandplan.outline.StaturoryDaily.DailyEntity";
			json.entityTwo = "com.cgn.itmp.outlineandplan.outline.NonStaturoryDaily.DailyEntity";
			json.base = jdid;
			json.crew = jzid;
			json = nui.encode(json);
			$.ajax({
			    url:"com.cgn.itmp.outlineandplan.outline.common.CommonServiceImplFlow.getDataByDetailed.biz.ext",
			    type:'POST',
			    cache:false,
			    data:json,
			    contentType:'text/json',
			    success:function(data){
			    	 if(data.record.data1){
			    	 	var url = '/itmp/outline/Staturory/Daily/index.jsp?detailed='+encodeURI(JSON.parse(JSON.stringify(detailed)),"utf-8");
			    	 	addTab(detailed,'法定',url);
			    	 }
			    	 if(data.record.data2){
			    	 	var url = '/itmp/outline/NonStaturory/Daily/index.jsp?detailed='+encodeURI(JSON.parse(JSON.stringify(detailed)),"utf-8");
			    	 	addTab(detailed,'非法定',url);
			    	 }
			    	 
			    	 if(!data.record || (!data.record.data1 && !data.record.data2)){
			    	 	nui.alert("无相关数据","提示");
			    	 }
		   		}
			});
		...
		function addTab(detailed,type,url) {
	         var tabs = parent.parent.nui.get("mainTabs");
	         var tab = { title: type + "日常执行条目", url: url, showCloseButton: true };
	         tab = tabs.addTab(tab);            
		     tabs.activeTab(tab);
	    }
         </script>
