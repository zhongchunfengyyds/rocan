...
- Description:
    ...
    <body style="width:98%;height:95%;margin:0 auto;">
        <div class="nui-panel" title="查询"  style="width:100%;height:25%;" showToolbar="false" showFooter="true">
            <div id="recordFromList" class="nui-form" align="center" style="height:100%">
                <!-- 数据实体的名称 -->
                <input class="nui-hidden" name="criteria/_entity" value="com.cgn.itmp.outlineandplan.outline.common.curriculumVitae.Dgglvitae8">
                <!-- 排序字段 -->
                                	...
                                	if(row.type == '新增' || row.type == '修改'){
                                		if (row) {
	                                		 nui.open({showMaxButton : true,
									        	...
									        	var urlStr = "com.cgn.itmp.outlineandplan.outline.common.CommonServiceImplFlow.getViewInforMation.biz.ext";
										        $.ajax({
											        url:urlStr,
											        type:'POST',
											        data:parms,
    ...
    function output(){
    	...
    	nui.open({
	        ...
	        var iframe = this.getIFrameEl();
	        var data = {
	                entityName:"com.cgn.itmp.outlineandplan.outline.common.curriculumVitae.Dgglvitae8",
