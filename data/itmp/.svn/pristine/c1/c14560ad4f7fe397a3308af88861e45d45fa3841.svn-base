		...
		<div class="search-container folding-menu" id="queryForm">
			<div style="width: 100%; height: 100%;">
				...
				<div id="form1" class="nui-form display" align="center" style="height: 27px">
					<!-- 数据实体的名称 -->
					<input class="nui-hidden" name="criteria/_entity" value="com.cgn.itmp.dailyandoverhaul.overhaul.RiskMgt.RiskMgtSearch">
					<!-- 排序字段 -->
...
<!-- 						</ul> -->
		...
		<div class="nui-fit">
			<div id="datagrid1" dataField="tddxglfxgls" class="nui-datagrid"
				style="width: 100%; height: 100%;"
				url="com.cgn.itmp.dailyandoverhaul.overhaul.RiskMgt.queryTdDxglFxgls.biz.ext"
				...
				<div property="columns">
					...
					<div field="type1" width="100" headerAlign="center"
						renderer="typeRenderer">
						类型 <input property="editor" class="nui-dictcombobox"  required="true" requiredErrorText="类型 不能为空"
							style="width: 100%;" dictTypeId="DXGL_TYPE" />
					</div>
					...
					<div field="bc" allowResize="false" width="120"
						headerAlign="center" allowSort="true">
						版次 <input property="editor" class="nui-textbox"
							style="width: 100%;" />
					</div>
			...
			tip3.set({
			 ...
			 function output(){
                ...
                nui.open({showMaxButton : true,
                    ...
                    var data = {
                            entityName:"com.cgn.itmp.dailyandoverhaul.overhaul.RiskMgt.RiskMgtSearch",
                            queryParam:json,								    //查询数据
                            queryKey:'criteria',                              //查询实体名
                            query:['xh','isdelete','jdid','updtime'],  //按顺序放置
                            filterField:['fileRelativePath','jdid','jzid','orderindex','uuid','isdelete','fj','updtime','upduser','dxlc','fildid','fileId','fileRealName','filePath','fileSize','fileType','fileUploadTime','fileUploadUser','fileNewName'],
                            expTitle:"风险管理",
                            dicFieds : "type1@DXGL_TYPE",
                            jdFieds:"jdid",
                        	jzFieds:"jzid",
			...
			function batchAdd(){
			 nui.open({showMaxButton : true,
                    ...
                    onload: function () {//弹出页面加载完成
	                    ...
	                    var data = {
	                    	pageType:"batchAdd",
	                    	tempFileName:"dxfxglImport.xlsx",
	                    	tempLoadName :"风险管理导入模板",
	                    	entitfName:"com.cgn.itmp.dailyandoverhaul.overhaul.RiskMgt.RiskMgt"
	                    };//传入页面的json数据
           ...
           //点击附件下载
			...
			function onCellValidation(e) {
	         if(e.field == "jzid") {
	             ...
	             if (e.field == "jzid"||e.field == "dxlc"||e.field == "type1"  || e.field == "fj" || e.field == "xh") {
			...
			grid.commitEdit();
				...
				if(jsonObject != null && jsonObject.length > 0){
					jsonObject[0]['filename'] = fileName;
			 		...
			 		jsonObject[0]['name'] = fileName.split('.')[0];
				}
			 	var data = {
	            ...
	            nui.ajax({
	                url: "com.cgn.itmp.dailyandoverhaul.overhaul.RiskMgt.newAddEdit.biz.ext",
	                type: 'POST',
	                data: json,
                    ...
                    //回收站页面
            function recycle() {
                nui.open({showMaxButton : true,//http://127.0.0.1:8080/itmp/dxgcgl/riskcontroll/HuiShouTdDxglFxglList.jsp
                    url: "/itmp/overhaul/RiskMgt/RiskMgtRecycle.jsp",
           ...
           function remove2(){
             var rows = grid.getSelecteds();
                if(rows.length > 0){
                    ...
                    function(action){
                        if(action=="ok"){
                            var json = nui.encode({tddxglfxgls:rows,token:"<%=TokenUtil.getTokenString(request.getSession())%>"});
                            grid.loading("正在删除中,请稍等...");
                            $.ajax({// com.cgn.itmp.dailyandoverhaul.overhaul.RiskMgt.LogicDeleteTdDxglFxgls
                                url:"com.cgn.itmp.dailyandoverhaul.overhaul.RiskMgt.LogicDeleteTdDxglFxgls.biz.ext",
                                type:'POST',
                                data:json,
                                cache: false,
		         ...
		         //轮次
		            function lcRenderer(e){
						return nui.getDictText("DXLC",e.value);
					}
					
		        //类型
		            function typeRenderer(e){
						return nui.getDictText("DXGL_TYPE",e.value);
					}
