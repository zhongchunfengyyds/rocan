    ...
    <body style="width:98%;height:95%;">
        <div class="nui-panel" title="查询"   style="width:100%;height:25%;" showToolbar="false" showFooter="true">
            <div id="recordFromList" class="nui-form" align="center" style="height:100%">
                <!-- 数据实体的名称 -->
                <input class="nui-hidden" name="criteria/_entity" value="com.cgn.itmp.outlineandplan.outline.common.curriculumVitae.Dgglvitae5">
                <!-- 排序字段 -->
                <!-- 排序字段 -->
			...
			nui.get("jz").setValue(jzid);
			                    ...
			                    function showdetail() {
			                    	...
			                    	if(row.type == '新增' || row.type == '修改'){
			                    		urlStr = "/itmp/tenyearplan/NuclearIsland/Daily/addForm.jsp?flag=1";
			                    		if (row) {
			                    		 nui.open({showMaxButton : true,
									        url: urlStr,
									        title: "数据详情",
									        ...
									        onload: function () {
									        	var iframe = this.getIFrameEl();
									        	var parms = '{"parms":{"id":"'+row.planid+'","name":"","entity":"com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandDaily.DailyEntity"}}';
										        ...
										        $.ajax({
											        ...
											        success:function(data){
											        	...
											        	if(returnJson.record[0]){
											        		var data = {pageType:"view",record:{tdgglsnjhhdrc:returnJson.record[0]}};
								                            //直接从页面获取，不用去后台获取
								                            iframe.contentWindow.setFormData(data);
											        	}
											        }
                                		...
                                		urlStr = "/itmp/tenyearplan/NuclearIsland/Daily/vitaeList.jsp";
                                		if (row) {
	                                		 nui.open({showMaxButton : true,
										        url: urlStr,
										        title: "数据详情",
                                		...
                                		urlStr = "com.cgn.itmp.basecommon.planComplateFlow.flow?flag=1&processInstId="+processInstId+"&showPage="+showPage+"&planFlag=1&modelFlag=6&transferEntity=com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandOverhaulEdit.NuclearIslandEdit";
                                		if (row) {
	                                		 nui.open({
	                                		 	showMaxButton : true,
										        url: urlStr,
                                		...
                                		var processInstId = row.planid;
                                		var showPage = "/basecommon/workFolw/planCompleteFirst.jsp";
                                		urlStr = "com.cgn.itmp.basecommon.planComplateFlow.flow?flag=1&processInstId="+processInstId+"&showPage="+showPage+"&planFlag=2&modelFlag=6&transferEntity=com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandOverhaulEdit.NuclearIslandEdit";
                                		if (row) {
	                                		 nui.open({
	                                		 	showMaxButton : true,
										        url: urlStr,
			            ...
			            function setFormData(data){
			            	...
			            	nui.open({
						        ...
						        var iframe = this.getIFrameEl();
						        var data = {
						                entityName:"com.cgn.itmp.outlineandplan.outline.common.curriculumVitae.Dgglvitae5",
