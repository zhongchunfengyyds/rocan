...
<body style="background-color:#f1f1f1;width: 98%; height:97%;">
				...
				<div style="width: 100%; height: 100%;">
					...
					<div id="form1" class="nui-form displaynone" align="center"
						...
						<input class="nui-hidden" name="criteria/_entity"
							value="com.cgn.itmp.inspectcomponentmanage.insplanmg.Insplanmg">
						<!-- 排序字段 -->
						<table id="table1" class="table" style="height: 100%">
							...
							<tr
								...
								<td style="visibility: hidden; height: 1px; overflow: hidden;"
									...
									name="criteria/_expr[4]/isdel" value="0" /> <input
	...
	<div style="width: 100%; height: 94%;" showToolbar="false"
		...
		<div class="nui-fit">
			<div id="datagrid1" dataField="insplanmgs" class="nui-datagrid"
				style="width: 100%; height: 100%;"
				url="com.cgn.itmp.inspectcomponentmanage.insplanmg.insplanmgbiz.queryInsplanmgs.biz.ext"
				...
				<div property="columns">
					...
					<div field="fj" headerAlign="center" allowSort="true"
						renderer="setFile">冲洗后管间检查计划</div>
...
<%-- 		accessControlButton('<%=userRoleCodeList%>',7); --%>
		...
		function add() {
			nui.open({
						showMaxButton : true,
						url : "/itmp/inspectcomponentmanage/insplanmg/InsplanmgForm.jsp",
						title : "新增记录",
						width : 600,
						height : 200,
								...
								function(action) {
									if (action == "ok") {
										var json = nui
												.encode({
													data : rows,
													entity : "com.cgn.itmp.inspectcomponentmanage.insplanmg.Insplanmg",token:"<%=TokenUtil.getTokenString(request.getSession())%>"
												});
...
// 			nui.get("update").enable();
			...
			nui
					.open({
						...
						onload : function() {//弹出页面加载完成
							...
							var data = {
								pageType : "recyclebin",
								urlName : "com.cgn.itmp.inspectcomponentmanage.insplanmg.insplanmgbiz.queryInsplanmgs.biz.ext",//回收
								entityVal : "com.cgn.itmp.inspectcomponentmanage.insplanmg.Insplanmg",
		...
		function btnQuery() {
			...
			if (gjz == "") {
				...
				query["criteria/_expr[9]/funname"] = "";
		...
		function record() {
			nui.open({
				showMaxButton : true,
				url : "/itmp/inspectcomponentmanage/insplanmg/InsplanmgListRecord.jsp",
				title : "修改记录",
				width : 1200,
				height : 630,
			});
		...
		function batchAdd() {
			nui
					...
					$("#delebtn").prop("disabled", "disabled");
							...
							var data = {
								pageType : "batchAdd",
								tempFileName : "Insplanmg.xlsx",
								tempLoadName : "检查计划管理",
								entitfName : "com.cgn.itmp.inspectcomponentmanage.insplanmg.Insplanmg"
							};//传入页面的json数据
			...
			nui
					.open({
						...
						onload : function() {//弹出页面加载完成
							var iframe = this.getIFrameEl();
							var data = {
								entityName : "com.cgn.itmp.inspectcomponentmanage.insplanmg.Insplanmg",
								queryParam : json, //查询数据
								queryKey : 'criteria', //查询实体名
								query : [  'overhaulsq', 'insphase', 'insscope', 'isdel', 'jd', 'jz' ], //按顺序放置
          	...
          	if(data){
          		for(var i=0;i<data.length;i++){
          			if(data[i].processStatus=="0"){
		          		focds+="检查阶段["+data[i].insphase+"]正在[修改]流程中<br>";
          			}
          		}
          	}
          	if(focds==""){
