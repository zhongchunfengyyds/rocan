...
- Description:
    ...
    <body style="width:98%;height:97%;">
        <div class="nui-panel" title="年度计划常规岛及BOP日常计划" style="width:100%;height:auto;" showToolbar="false" showFooter="true">
            <div id="form1" class="nui-form" align="left" style="height:100%">
                <!-- 数据实体的名称 -->
                <input class="nui-hidden" name="criteria/_entity" value="com.cgn.itmp.outlineandplan.annual.ConventionalIslandDailyPlan.PlanEntity">
                <!-- 排序字段 -->
            ...
            <a id="del" style="display: none"  class="nui-button btn btn-default btn-xs" onclick="onDel()">
             ...
             <a class="nui-button btn btn-default btn-xs" id="effect" onclick="effect()">
              		 生效
            </a>
        ...
        <!--footer-->
         <div title="center" region="center" style="width:100%;height:90%;">
            <div class="nui-fit">
                <div 
                        id="datagrid1"
                        dataField="tgglndjhcgdrcs"
                        class="nui-datagrid"
                        style="width:100%;height:100%;"
                        url="com.cgn.itmp.outlineandplan.annual.ConventionalIslandDaily.queryTgglndjhcgdrcs.biz.ext"
                    ...
                    <div property="columns">
                        ...
                        <div field="ndplan" renderer="setdanhao" width="220" headerAlign="center" allowSort="true" >
                            年度计划
                        </div>
                        ...
                        <div field="creatdate" headerAlign="center" allowSort="true" dateFormat="yyyy-MM-dd HH:mm:ss">
                         ...
                         <div field="isdelete" headerAlign="center" width="250" renderer="downloadFile">
                           生效文件
                        </div>
                    </div>
                </div>
				...
				search();//grid查询
                                ...
                                function createplan(){
                                	...
                                	if(lc == ""){
                                		...
                                		json.crew = jzid;
                                		json.lc = lc;
                                		json.ident = "年度计划常规岛日常";
                                		json.entity = "com.cgn.itmp.outlineandplan.outline.common.curriculumVitae.Dgglvitae11";
                                		...
                                		$.ajax({
							                ...
							                success:function(data){
							                	if(data.record){
							                		var jsondata = '{"data":{"ident":"年度计划常规岛日常","baseid":"'+jdid+'","crewid":"'+jzid+'","lc":"'+lc+'","username":"'+'<%= username%>'+'"}}';
			        								var urlStr = "com.cgn.itmp.outlineandplan.annual.ConventionalIslandDaily.createNDJHRC.biz.ext";
							                		...
							                		nui.alert("该年度计划数据已存在,无需重新生成","提示");
							                	}
							                }
							            });
                                	}
					           ...
					           //超链接页面  
					            function showdetail() {
					                ...
					                if (row) {
					                    nui.open({showMaxButton : true,
					                        url: "/itmp/annual/ConventionalIsland/Daily/linkList.jsp",
					                        title: "数据详情",
					                        width: 1000,
					                        height: 600,
					                        onload: function () {
					                             var iframe = this.getIFrameEl();
								                 var data = {pageType:"data",record:{ndjhcgd:row}};
								                //直接从页面获取，不用去后台获取
			                    ...
			                    function showrecorddetail(){
					                ...
					                if (row) {
					                	nui.open({showMaxButton : true,
					                        url: "/itmp/annual/ConventionalIsland/Daily/linkDetail.jsp",
					                        title: "数据详情",
					                        width: 800,
					                        height: 550,
					                        onload: function () {
					                             var iframe = this.getIFrameEl();
								                 var data = {pageType:"data",record:{ndplan:row.ndplan,crew:row.crew,base:row.base,lc:row.lc,ident:'年度计划常规岛日常'}};
								                //直接从页面获取，不用去后台获取
								                iframe.contentWindow.setFormData(data);
				                            }
			                            });
	...
	if(rows.length > 0){
		...
		function(action){
            if(action=="ok"){
                ...
                json.entity='com.cgn.itmp.outlineandplan.annual.ConventionalIslandDailyPlan.PlanEntity';
                json.mxEntity='com.cgn.itmp.outlineandplan.annual.ConventionalIslandDaily.PlanGeneration';
	                ...
	                success:function(text){
                        ...
                        if(returnJson.record == 'true'){
							...
							if(row[0].isdelete == "4"){
								...
								return;
							}else if(row[0].isdelete == "3"){
								nui.alert("生效流程审批中，请等待流程结束！","提示");
								return;
							}
							
							nui.open({
							showMaxButton : true,
							 url : '/itmp/annual/ConventionalIsland/Daily/effectList.jsp',
							 title :'生效',
					 		 width: '90%',
			                 height: '90%',
			                 onload: function () {
		 ...
		 function downloadFile(e){
          ...
          function expotyPDF(id){
         		var json = nui.encode({
         					id: id,
         					entityName:'com.cgn.itmp.outlineandplan.annual.ConventionalIslandDailyPlan.PlanEntity',
         					childEntityName:'com.cgn.itmp.outlineandplan.annual.ConventionalIslandDaily.PlanGeneration'
         				});
	                ...
	                success: function (text) {
	                	if(text.result != null){
	                		nui.hideMessageBox(message);
	                     	openDownloadDialog("<%=request.getContextPath()%>/"+encodeURI(text.result),text.result);
	                	}
	                },
