 ...
 * $log$
 */
public class ExecutiveEntriesFFDDXServiceImpl extends DASDaoSupport implements
		ExecutiveEntriesFFDDXService {

	/**
	 * 
	 */
	@Bizlet
	public ExecutiveEntriesFFDDXServiceImpl() {
		// TODO 自动生成的构造函数存根
	}
	...
	public DataObject[] getProtypeAndPartsData(String baseid,String crewid) {
		// TODO 自动生成的方法存根
		String address = "com.cgn.itmp.outlineandplan.outline.common.dxzxtm.getProtype3";
	...
	public void addInfoMation(DataObject[] data) {
		...
		DataObject temp  = DataObjectUtil.convertDataObject(data[0], "com.cgn.itmp.outlineandplan.outline.NonStaturoryOverhaul.OverhaulEntity", false);
		temp.setString("id", uuid);
		DatabaseExt.insertEntityCascade("default", temp);
		String protype = temp.getString("protype");
		this.addRecordInfoMation(temp,"新增规范非法定大纲:"+protype+"_大修执行条目1条","新增",uuid);
	}
	@Bizlet
	public void addData(DataObject data) {
		...
		if(data == null) return;
		String uuid = UUID.randomUUID().toString().replaceAll("-", "");
		DataObject temp  = DataObjectUtil.convertDataObject(data, "com.cgn.itmp.outlineandplan.outline.common.NonStaturory.NonStaturoryEdit", false);
		temp.setString("id", uuid);
		DatabaseExt.insertEntityCascade("default", temp);
	...
	public void deleteRecoversData(DataObject[] data) {
		...
		if(!StringHelper.isEmpty(ids)){
			...
			for (int i = 0; i < idArr.length; i++) {
				String id = idArr[i];
				String address = "com.cgn.itmp.outlineandplan.outline.common.dxzxtm.deleteRecoversData3";
	...
	public void addEditionInfoMation(DataObject[] data){
		if(data == null) return;
		String uuid = UUID.randomUUID().toString().replaceAll("-", "");
		DataObject temp  = DataObjectUtil.convertDataObject(data[0], "nonstatutory.ascendingedition.Dggldgffdedition", false);
		temp.setString("id", uuid);
		DatabaseExt.insertEntityCascade("default", temp);
	...
	public String getCountForEdition() {
		// TODO 自动生成的方法存根
		String address = "com.cgn.itmp.outlineandplan.outline.common.dxzxtm.getCountForEdition3";
	...
	public void addRecordInfoMation(DataObject temp,String content,String type,String uuid){
		...
		temp1.setString("ident", "非法定大修执行条目");
	...
	public void updateInfoMation(DataObject data) {
		...
		DataObject temp = DataObjectUtil.convertDataObject(data, "com.cgn.itmp.outlineandplan.outline.NonStaturoryOverhaul.OverhaulEntity", false);
		...
		if(!StringHelper.isEmpty(parms)){
			if("remove".equals(parms)){
				this.addRecordInfoMation(temp,"删除规范非法定大纲:"+protype+"_大修执行条目1条","删除",id);
				...
				if(!StringHelper.isEmpty(updata)){
					DataObject dataEntity = DataObjectUtil.createDataObject("com.cgn.itmp.outlineandplan.outline.common.NonStaturory.NonStaturoryEdit");
					...
					while(keys.hasNext()){
						...
						Field modeField = new ViewModelHelper().getField("com.cgn.itmp.outlineandplan.outline.NonStaturoryOverhaul.OverhaulEntity", key);
						if(modeField != null){
	...
	public Map<String,Object> synchronous(DataObject data){
		...
		if(typeDataObject != null && typeDataObject.length>0){
			...
			if(stateMap != null && stateMap.size()>0){
				DataObject dataEntity = DataObjectUtil.createDataObject("com.cgn.itmp.outlineandplan.outline.NonStaturoryOverhaul.OverhaulEntity");
				for (int i = 1; i <= 60; i++) {
