 ...
 * $log$
 */
public class ExecutiveEntriesDailyServiceImpl extends DASDaoSupport implements
		ExecutiveEntriesDailyService {

	/**
	 * 
	 */
	@Bizlet
	public ExecutiveEntriesDailyServiceImpl() {
		// TODO 自动生成的构造函数存根
	}
	...
	public DataObject[] getProtypeAndPartsData(String baseid,String crewid) {
		// TODO 自动生成的方法存根
		String address = "com.cgn.itmp.outlineandplan.outline.common.dxzxtm.getProtype2";
	...
	public Map<String, Object> getBaseCircle(String baseid,String crewid) {
		...
		parmsMap.put("baseid", baseid);
		parmsMap.put("crewid", crewid);
		String address = "com.cgn.itmp.outlineandplan.outline.common.dxzxtm.getRCBaseCircle";
		Object[] typeDataObject = (Object[])DatabaseExt.queryByNamedSql("default", address, parmsMap);
	...
	public void addInfoMation(DataObject[] data) {
		...
		DataObject temp  = DataObjectUtil.convertDataObject(data[0], "com.cgn.itmp.outlineandplan.outline.StaturoryDaily.DailyEntity", false);
		temp.setString("id", uuid);
		DatabaseExt.insertEntityCascade("default", temp);
		String protype = temp.getString("protype");
		this.addRecordInfoMation(temp,"新增规范法定大纲:"+protype+"_日常执行条目1条","新增",uuid);
	}
	...
	public void deleteRecoversData(DataObject[] data) {
		...
		if(!StringHelper.isEmpty(ids)){
			...
			for (int i = 0; i < idArr.length; i++) {
				String id = idArr[i];
				String address = "com.cgn.itmp.outlineandplan.outline.common.dxzxtm.deleteRecoversData2";
	...
	public void addEditionInfoMation(DataObject[] data){
		if(data == null) return;
		String uuid = UUID.randomUUID().toString().replaceAll("-", "");
		DataObject temp  = DataObjectUtil.convertDataObject(data[0], "daily.ascendingedition.Dggledition2", false);
		temp.setString("id", uuid);
		DatabaseExt.insertEntityCascade("default", temp);
//		String number = this.getCountForEdition();
	...
	public String getCountForEdition() {
		// TODO 自动生成的方法存根
		String address = "com.cgn.itmp.outlineandplan.outline.common.dxzxtm.getCountForEdition2";
	...
	public void updateInfoMation(DataObject data) {
		...
		DataObject temp = DataObjectUtil.convertDataObject(data, "com.cgn.itmp.outlineandplan.outline.StaturoryDaily.DailyEntity", false);
		...
		if(!StringHelper.isEmpty(parms)){
			if("remove".equals(parms)){
				this.addRecordInfoMation(temp,"删除规范法定大纲:"+protype+"_日常执行条目1条","删除",id);
				...
				if(!StringHelper.isEmpty(updata)){
					DataObject dataEntity = DataObjectUtil.createDataObject("daily.fdrcedit.Tdggldailyedit");
					...
					while(keys.hasNext()){
						...
						Field modeField = new ViewModelHelper().getField("com.cgn.itmp.outlineandplan.outline.StaturoryDaily.DailyEntity", key);
						if(modeField != null){
	...
	public Map<String,Object> synchronous(DataObject data){
		...
		if(typeDataObject != null && typeDataObject.length>0){
			...
			if(stateMap != null && stateMap.size()>0){
				DataObject dataEntity = DataObjectUtil.createDataObject("com.cgn.itmp.outlineandplan.outline.StaturoryDaily.DailyEntity");
