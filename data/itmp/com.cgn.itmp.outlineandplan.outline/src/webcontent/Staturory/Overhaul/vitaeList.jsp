    ...
    <body style="width:98%;height:95%;margin:0 auto;">
        <div class="nui-panel" title="查询"  style="width:100%;height:25%;" showToolbar="false" showFooter="true">
            <div id="recordFromList" class="nui-form" align="center" style="height:100%">
                <!-- 数据实体的名称 -->
                <input class="nui-hidden" name="criteria/_entity" value="com.cgn.itmp.outlineandplan.outline.common.curriculumVitae.Dgglvitae">
                <!-- 排序字段 -->
           ...
           变更时间:             	
                        ...
                        <td id="ctimetd">
		                    ...
		                    <input id="startDate" class="nui-datepicker" allowInput="false" showClose="true" oncloseclick="this.setValue(null);" ondrawdate="onDrawDate" name="criteria/_expr[7]/datetime"/>
		                    <input class="nui-hidden" name="criteria/_expr[7]/_op" value=">">--
		                    <input id="endDate" class="nui-datepicker"  allowInput="false" showClose="true" oncloseclick="this.setValue(null);" ondrawdate="onDrawDate" name="criteria/_expr[8]/datetime"/>
            ...
            <div class="nui-fit">
                <div 
                    ...
                    <div property="columns">
                        ...
                        <div field="datetime" headerAlign="center" align="center" allowSort="true" dateFormat="yyyy-MM-dd HH:mm:ss" width="13%">
                            变更日期
                        </div>
			...
			var stime;
                                ...
                                function showdetail() {
                                	...
                                	if(row.type == '新增' || row.type == '修改'){
                                		urlStr = "/itmp/outline/Staturory/Overhaul/addForm.jsp?flag=1";
                                		if (row) {
	                                		 nui.open({showMaxButton : true,
									        	...
									        	var parms = '{"parms":{"id":"'+row.planid+'","name":"","entity":"com.cgn.itmp.outlineandplan.outline.StaturoryOverhaul.OverhaulEntity"}}';
										        ...
										        $.ajax({
											        ...
											        success:function(data){
											        	...
											        	if(returnJson.record[0]){
											        		var data = {pageType:"view",record:{tdgglentry:returnJson.record[0]}};
								                            //直接从页面获取，不用去后台获取
                                		...
                                		urlStr = "/itmp/outline/Staturory/Overhaul/vitaeList.jsp";
                                		if (row) {
	                                		 nui.open({showMaxButton : true,
										        url: urlStr,
										        title: "数据详情",
                                		...
                                		urlStr = "com.cgn.itmp.basecommon.planComplateFlow.flow?flag=1&processInstId="+processInstId+"&showPage="+showPage+"&planFlag=1&modelFlag=1&transferEntity=com.cgn.itmp.outlineandplan.outline.StaturoryOverhaul.OverhaulEdit";
                                		if (row) {
	                                		 nui.open({
	                                		 	showMaxButton : true,
										        url: urlStr,
                                		...
                                		var processInstId = row.planid;
                                		var showPage = "/basecommon/workFolw/planCompleteFirst.jsp";
                                		urlStr = "com.cgn.itmp.basecommon.planComplateFlow.flow?flag=1&processInstId="+processInstId+"&showPage="+showPage+"&planFlag=2&modelFlag=1&transferEntity=com.cgn.itmp.outlineandplan.outline.StaturoryOverhaul.OverhaulEdit";
                                		if (row) {
	                                		 nui.open({
	                                		 	showMaxButton : true,
										        url: urlStr,
			            ...
			            function output(){
			            	...
			            	nui.open({
						        ...
						        var iframe = this.getIFrameEl();
						        var data = {
						                entityName:"com.cgn.itmp.outlineandplan.outline.common.curriculumVitae.Dgglvitae",
						                queryParam:json,								    //查询数据
						                queryKey:'criteria',                              //查询实体名
						                query:['creator','datetime','datetime','reason','content','type','datetime','datetime','base','crew'],  //按顺序放置
						                expTitle:"规范法定大纲管理大修执行条目履历信息",
