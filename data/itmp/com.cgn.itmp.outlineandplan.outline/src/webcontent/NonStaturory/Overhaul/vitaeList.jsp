    ...
    <body style="width:98%;height:95%;">
        <div class="nui-panel" title="查询"   style="width:100%;height:25%;" showToolbar="false" showFooter="true">
            <div id="recordFromList" class="nui-form" align="center" style="height:100%">
                <!-- 数据实体的名称 -->
                <input class="nui-hidden" name="criteria/_entity" value="com.cgn.itmp.outlineandplan.outline.common.curriculumVitae.Dgglvitae2">
                <!-- 排序字段 -->
                            ...
                            function(action){
                                ...
                                function showdetail() {
                                	...
                                	if(row.type == '新增' || row.type == '修改'){
                                		urlStr = "/itmp/outline/NonStaturory/Overhaul/addForm.jsp?flag=1";
                                		if (row) {
                                		 nui.open({showMaxButton : true,
									        url: urlStr,
									        title: "数据详情",
									        ...
									        onload: function () {
									        	var iframe = this.getIFrameEl();
									        	var parms = '{"parms":{"id":"'+row.planid+'","name":"","entity":"com.cgn.itmp.outlineandplan.outline.NonStaturoryOverhaul.OverhaulEntity"}}';
										        ...
										        $.ajax({
											        ...
											        success:function(data){
											        	...
											        	if(returnJson.record[0]){
											        		var data = {pageType:"view",record:{tgglgfffddxtm:returnJson.record[0]}};
								                            //直接从页面获取，不用去后台获取
								                            iframe.contentWindow.setFormData(data);
											        	}
											        }
                                		...
                                		urlStr = "/itmp/outline/NonStaturory/Overhaul/vitaeList.jsp";
                                		if (row) {
	                                		 nui.open({showMaxButton : true,
										        url: urlStr,
										        title: "数据详情",
                                		...
                                		urlStr = "com.cgn.itmp.basecommon.planComplateFlow.flow?flag=1&processInstId="+processInstId+"&showPage="+showPage+"&planFlag=1&modelFlag=3&transferEntity=com.cgn.itmp.outlineandplan.outline.common.NonStaturory.NonStaturoryEdit";
                                		if (row) {
	                                		 nui.open({
	                                		 	showMaxButton : true,
										        url: urlStr,
                                		...
                                		var processInstId = row.planid;
                                		var showPage = "/basecommon/workFolw/planCompleteFirst.jsp";
                                		urlStr = "com.cgn.itmp.basecommon.planComplateFlow.flow?flag=1&processInstId="+processInstId+"&showPage="+showPage+"&planFlag=2&modelFlag=3&transferEntity=com.cgn.itmp.outlineandplan.outline.common.NonStaturory.NonStaturoryEdit";
                                		if (row) {
	                                		 nui.open({
	                                		 	showMaxButton : true,
										        url: urlStr,
            ...
            e.allowSelect = false;
					            ...
					            function output(){
					            	...
					            	nui.open({
								        ...
								        var iframe = this.getIFrameEl();
								        var data = {
								                entityName:"com.cgn.itmp.outlineandplan.outline.common.curriculumVitae.Dgglvitae2",
								                queryParam:json,								    //查询数据
								                queryKey:'criteria',                              //查询实体名
								                query:['creator','datetime','datetime','reason','content','type','datetime','datetime','base','crew'],  //按顺序放置
								                expTitle:"规范非法定总纲管理大修执行条目履历信息",
