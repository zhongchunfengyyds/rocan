		...
		<div class="search-container folding-menu" id="queryForm">
			<div style="width: 100%; height: 100%;">
				...
				<div id="overhaulform" class="nui-form display" align="center" style="height: 16%">
					<!-- 数据实体的名称 -->
					<input class="nui-hidden" name="criteria/_entity" value="com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandOverhaul.OverhaulSearch">
					<!-- 排序字段 -->
      ...
      <div class="nui-layout" style="width:100%;height:100%;" > 
        ...
        <div class="nui-fit">
        	<ul id="tree1" expandOnNodeClick="true" class="nui-tree" 
        		resultAsTree="false" style="width:100%;height:100%;" idField="id" parentField="parentid" 
    	...
    	<div title="center" region="center" >
            ...
            <div class="nui-fit">
                <div 
                        id="datagrid1"
                        dataField="tdgglsnjhhddxs"
                        class="nui-datagrid"
                        style="width:100%;height:100%;"
                        url="com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandOverhaul.queryTdgglsnjhhddxs.biz.ext"
                        pageSize="25"
                    ...
                    <div property="columns">
                        ...
                        <div field="base" headerAlign="center" visible="false" allowSort="true" visible="false" Align="center">
                            基地
                        </div>
                        <div field="crew" headerAlign="center" visible="false" allowSort="true" visible="false" Align="center">
                            机组
                        </div>
                        <div field="protype" headerAlign="center" visible="false" allowSort="true" visible="false" Align="center">
                            项目类型
                        </div>
                        <div id="parts"  name="parts" field="parts" visible="false" headerAlign="center" allowSort="true" Align="center">
                            设备/系统
                        </div>
	...
	$.ajax({
	    ...
	    success:function(data){
	    	...
	    	if(returnJson.data.sucess){
	            ...
	            nui.get('tree1').setUrl("com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandOverhaul.getProtypeAndPartsData.biz.ext?baseid="+jdid+"&crewid="+jzid);
// 	            searchs();
				for(var i = 1;i <=  60 ;i++){
						var colName = "c"+i;
...
tree.on("nodeselect", function (e) {
    if (e.isLeaf) {
       //grid.load({"criteria/_entity": "com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandOverhaul.OverhaulEntity","criteria/_expr[4]/parts" : e.node.name });
...
function add() {
	...
	if (row) {
		nui.open({showMaxButton : true,
	        url: "/itmp/tenyearplan/NuclearIsland/Overhaul/addForm.jsp",
	        title: "新增记录", width: 800, height: 400,
	        onload: function () {//弹出页面加载完成
		        var iframe = this.getIFrameEl();
		        var data = {pageType:"add",record:{tdgglsnjhhddx:row},parts:row.parts,protype:row.protype};//传入页面的json数据
    	...
    	$.ajax({
                ...
                success:function(data){
                	...
                	if(returnJson.record != ''){
                		...
                		nui.open({showMaxButton : true,
				            url: "/itmp/tenyearplan/NuclearIsland/Overhaul/editForm.jsp",
				            title: "编辑数据",
				            width: 800,
				            height: 400,
				            onload: function () {
...
function openrecord(){
	nui.open({showMaxButton : true,
            url: "/itmp/tenyearplan/NuclearIsland/Overhaul/vitaeList.jsp",
            title: "变更履历",
            width: 1000,
            height: 520,
            onload: function () {
...
function synchronous(){
	var row = grid.getSelected();
    if (row) {
    	var jsondata = '{"tdgglsnjhhddx":'+ nui.encode(row)+'}';
    	var urlStr = "com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandOverhaul.synchronous.biz.ext";
		$.ajax({
            url:urlStr,
            type:'POST',
            data:jsondata,
                		...
                		if(isadmin == 'true'){//管理员直接删除
						  nui.confirm("确定删除选中记录？","系统提示",
						            ...
						            onload: function () {
							                ...
							                ,'processInstName':'十年计划(核岛)_大修执行条目_删除','processInstDesc':'十年计划(核岛)_大修执行条目_删除','entityType':'com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandOverhaulEdit.NuclearIslandEdit','modelFlag':5}};
							                //直接从页面获取，不用去后台获取
							                iframe.contentWindow.setFormData(data);
				            },
				            ondestroy: function (action) {
...
function recover(){
	nui.open({showMaxButton : true,
            url: "/itmp/tenyearplan/NuclearIsland/Overhaul/recycleList.jsp",
            title: "回收站查询",
            width: 800,
            height: 400,
            onload: function () {
	   		 ...
	   		 var count = data.count;
	   			if(count > 0){
	   				nui.alert("该基地升版流程正在审批中,不能升版","提示");
						...
						nui.open({
							showMaxButton : true,
				            url: "/itmp/outlineCommon/dgEdition/edtionStart.jsp?modelFlag=5&entity=com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandOverhaul.OverhaulEntity&basename=base&crewname=crew&editionname=edition",
				            title: "升版",
				            width: 500,
				            height: 330,
				            onload: function () {
...
function output(){
    ...
    nui.open({showMaxButton : true,
        ...
        var iframe = this.getIFrameEl();
        var data = {
                entityName:"com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandOverhaul.OverhaulSearch",
                queryParam:json,								    //查询数据
                queryKey:'criteria',                              //查询实体名
                query:['itemno','site','detailed','parts','protype','base','crew'],  //按顺序放置
                dicFieds: "protype@PROTYPE",
                expTitle:"十年计划核岛大修执行条目",
...
function batchAdd(){
	...
	$.ajax({
        ...
        onload: function () {//弹出页面加载完成
            ...
            var data = {
            	pageType:"batchAdd",
            	tempFileName:"snhddxzxtm.xlsx",
            	tempLoadName:"十年计划核岛部件大修执行条目导入模板",
            	entitfName:"com.cgn.itmp.outlineandplan.tenyearplan.NuclearIslandOverhaul.OverhaulEntity"
            };//传入页面的json数据
...
//关键字
		...
		function showdetail(detailed){
			...
			json.entityOne = "com.cgn.itmp.outlineandplan.outline.StaturoryOverhaul.OverhaulEntity";
			json.entityTwo = "com.cgn.itmp.outlineandplan.outline.NonStaturoryOverhaul.OverhaulEntity";
			json.base = jdid;
			json.crew = jzid;
			json = nui.encode(json);
			$.ajax({
			    ...
			    success:function(data){
			    	 if(data.record.data1){
			    	 	var url = '/itmp/outline/Staturory/Overhaul/index.jsp?detailed='+encodeURI(JSON.parse(JSON.stringify(detailed)),"utf-8");
			    	 	addTab(detailed,'法定',url);
			    	 }
			    	 if(data.record.data2){
			    	 	var url = '/itmp/outline/NonStaturory/Overhaul/index.jsp?detailed='+encodeURI(JSON.parse(JSON.stringify(detailed)),"utf-8");
		...
		function addTab(detailed,type,url) {
	         ...
	         var tab = { title: type + "大修执行条目", url: url, showCloseButton: true };
	         tab = tabs.addTab(tab);            
		     tabs.activeTab(tab);
	    }
         </script>
