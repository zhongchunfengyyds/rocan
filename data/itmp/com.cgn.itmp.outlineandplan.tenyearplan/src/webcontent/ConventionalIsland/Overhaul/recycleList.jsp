...
- Description:
    ...
    <body style="width:98%;height:95%;margin:0 auto;">
      	 <div id="form1"> 
                <!-- 数据实体的名称 -->
                <input class="nui-hidden" name="criteria/_entity" value="com.cgn.itmp.outlineandplan.tenyearplan.ConventionalIslandOverhaul.OverhaulRecycle">
                <!-- 排序字段 -->
            ...
            <div class="nui-fit">
                <div 
                    ...
                    <div property="columns">
                        ...
                        <div field="equipmentno" headerAlign="center" allowSort="true" Align="center">
                           设备编号/功能位置
                        </div>
                        <div field="equipment" headerAlign="center" allowSort="true" Align="center">
                           设备名称
                        </div>
...
function remove(){
    ...
    if(rows.length > 0){
        ...
        function(action){
            if(action=="ok"){
                ...
                $.ajax({
                    url:"com.cgn.itmp.outlineandplan.tenyearplan.RecoverMgt.deleteRecoversData7.biz.ext",
                    type:'POST',
                    data:jsondata,
                    cache: false,
	...
	function setDel(){
	    ...
	    if(rows.length > 0){
	        ...
	        function(action){
	            if(action=="ok"){
	                var json = {tdgglsnjhcgddxs:rows};
	                grid.loading("正在恢复,请稍等...");
	                var success = true;
	                for(var i = 0;i< json.tdgglsnjhcgddxs.length;i++){
	                	var jsondata = "{'tdgglsnjhcgddx':{'id':'"+json.tdgglsnjhcgddxs[i].uuid+"','isdelete':'0'}}";
	                	$.ajax({
		                    url:"com.cgn.itmp.outlineandplan.tenyearplan.ConventionalIslandOverhaul.updateTdgglsnjhcgddx.biz.ext",
		                    type:'POST',
		                    data:jsondata,
		                    cache:false,
		                    contentType:'text/json',
