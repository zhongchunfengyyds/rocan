...
- Description:
    ...
    <body style="width:98%;height:95%;margin:0 auto;">
      	 <div id="form1"> 
                <!-- 数据实体的名称 -->
                <input class="nui-hidden" name="criteria/_entity" value="com.cgn.itmp.outlineandplan.tenyearplan.ConventionalIslandDaily.DailyRecycle">
                <!-- 排序字段 -->
...
function remove(){
    ...
    if(rows.length > 0){
        ...
        function(action){
            if(action=="ok"){
                ...
                $.ajax({
                    url:"com.cgn.itmp.outlineandplan.tenyearplan.RecoverMgt.deleteRecoversData8.biz.ext",
                    type:'POST',
                    data:jsondata,
                    cache: false,
	...
	function setDel(){
	    ...
	    if(rows.length > 0){
	        ...
	        function(action){
	            if(action=="ok"){
	                var json = {tdgglsnjhcgdrcs:rows};
	                grid.loading("正在恢复,请稍等...");
	                var success = true;
	                for(var i = 0;i< json.tdgglsnjhcgdrcs.length;i++){
	                	var jsondata = "{'tdgglsnjhcgdrc':{'id':'"+json.tdgglsnjhcgdrcs[i].uuid+"','isdelete':'0'}}";
	                	$.ajax({
		                    url:"com.cgn.itmp.outlineandplan.tenyearplan.ConventionalIslandDaily.updateTdgglsnjhcgdrc.biz.ext",
		                    type:'POST',
		                    data:jsondata,
		                    cache:false,
		                    contentType:'text/json',
