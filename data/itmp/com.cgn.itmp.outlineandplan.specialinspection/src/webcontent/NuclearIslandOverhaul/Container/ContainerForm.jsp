...
<div id="dataform1" style="padding-top:5px;">
    ...
    <input class="nui-hidden" name="data.entitytype" value="com.cgn.itmp.outlineandplan.specialinspection.NuclearIslandOverhaul.Container"/>
    <!-- hidden域 流程对应信息 -->
    <input id='orderno' class="nui-hidden" name="data.orderno"/>
    <input class="nui-hidden" name="data.processDefName" value="com.cgn.itmp.outlineandplan.specialinspection.OverhaulRQRJHQAdd"/>
    <input class="nui-hidden" name="data.processInstName" value="专用检查计划(核岛部件)_大修执行条目_新增焊容器和热交换器"/>
    <input class="nui-hidden" name="data.processInstDesc" value="专用检查计划(核岛部件)_大修执行条目_新增焊容器和热交换器"/>
...
//  	nui.get('jhry').load(encodeURI("com.cgn.components.coframe.RoleService.getEmpListByRoleInfo.biz.ext?roleCode=ITMP_SPRY_" +  getCookie('jdname')));
    ...
    function saveData(nextStep) {
        ...
        //编辑
        if (pageType == "edit") {
            urlStr = "com.cgn.itmp.outlineandplan.specialinspection.tdggltzyjcjhhdrqbiz.updateTDgglTzyjcjhhdrq.biz.ext";
        }
        form.validate();
        ...
        if(isadmin == 'true'){
            data.data['entity'] = 'com.cgn.itmp.outlineandplan.specialinspection.NuclearIslandOverhaul.Container';
        }
