...
<!-- 						</ul> -->
		...
		<div class="nui-fit">
			<div id="datagrid1" dataField="tdxgldxgljhgls" class="nui-datagrid"
				...
				<div property="columns">
					...
					<div field="zzlx" width="100" headerAlign="center"
						...
						类型 <input property="editor" class="nui-dictcombobox"
							style="width: 100%;" dictTypeId="SEC_TYPE" />
					</div>
					<div field="zzmc" allowResize="false" width="120"
						headerAlign="center" allowSort="true">
						名称<input property="editor" id="zzmc" class="nui-textbox zzmc"
							readonly="readonly" style="width: 100%;" />
					</div>
					<div field="bc" allowResize="false" width="120"
						headerAlign="center" allowSort="true">
						版次<input property="editor" class="nui-textbox" style="width: 100%;" onvalidation="hasIllegalChar(e,50,true,true)"/>
					</div>
					<div field="fj" allowResize="false" width="120" renderer="setFile"
						headerAlign="center" allowSort="true">
						附件<input id="fj" property="editor" onvalidation="hasIllegalChar(e,50,true,true)"
					...
					<div field="upduser" allowResize="false" width="120"
					...
					<div field="addtime" width="100" allowSort="true"
						...
						上传日期 <input property="editor" class="nui-datepicker scsj"
							style="width: 100%;" readonly="readonly" />
					</div>

					<div field="yjjsr" headerAlign="center" allowSort="true"
						visible="false">收件人</div>
					<div field="updtime" headerAlign="center" allowSort="true"
						visible="false">添加时间</div>
					<div field="isdelete" headerAlign="center" allowSort="true"
						visible="false">isdelete</div>
				</div>
			</div>
		</div>
	</div>
	<script type="text/javascript">
		...
		function onCellValidation(e) {
			if (e.field == "jzid") {
				e.column.editor.url = "com.cgn.itmp.basecommon.DictionaryOperation.getJzDicInfoByjdid.biz.ext?jdid="
						+ getCookie('jdid');
			}
...
//                     grid.beginEditRow(e.row); 
		...
		function onUploadSuccess(e) {
			...
			var currentRow = e.sender.getParent().getEditingRow();

			// 		  	grid.updateColumn("zzmc", {header:"名称1"});

			// 		  	grid.updateRow(currentRow,{'zzmc':fileName,'fj':fileName});
...
nui.showTips({content: '表单校验不通过,请确认',state: 'warning',x: 'center', y: 'center'});return;			}
			...
			nui
					.ajax({
						url : "com.cgn.itmp.dailyandoverhaul.overhaul.PlanMgt.newAddEdit.biz.ext",
						type : 'POST',
						data : json,
		...
		function output() {
			...
			nui.open({
				...
				onload : function() {//弹出页面加载完成
					...
					var data = {
						entityName : "com.cgn.itmp.dailyandoverhaul.overhaul.PlanMgt.PlanMgtSearch",
						queryParam : json, //查询数据
						queryKey : 'criteria', //查询实体名
						query : [ 'xh', 'isdelete', 'jdid','addtime'], //按顺序放置
						filterField : ['fileRelativePath','jdid','jzid', 'orderindex','uuid', 'isdelete', 'fj', 'updtime',
								'upduser', 'yjjsr', 'dxlc', 'fildid', 'fileId',
								'fileRealName', 'filePath', 'fileSize',
								'fileType', 'fileUploadTime', 'fileUploadUser',
								'fileNewName' ],
						expTitle : "计划管理",
						dicFieds : "zzlx@SEC_TYPE",
						jdFieds : "jdid",
						jzFieds : "jzid",
			...
			nui
					.open({
						...
						onload : function() {//弹出页面加载完成
							...
							var data = {
								pageType : "batchAdd",
								tempFileName : "dxjhglImport.xlsx",
								tempLoadName : "计划管理导入模板",
								entitfName : "com.cgn.itmp.dailyandoverhaul.overhaul.PlanMgt.PlanMgt"
							};//传入页面的json数据
		...
		function recycle() {
			nui.open({
						showMaxButton : true,//
						url : "/itmp/overhaul/PlanMgt/PlanMgtRecycle.jsp",
						title : "回收站",
						width : 700,
						height : 400,
						onload : function() {
						...
						.confirm(
								...
								function(action) {
									if (action == "ok") {
										var json = nui.encode({
											data : rows,
											entity : "com.cgn.itmp.dailyandoverhaul.overhaul.PlanMgt.PlanMgt",token:"<%=TokenUtil.getTokenString(request.getSession())%>"
										});
