...
<div id="form1">
    <!-- 数据实体的名称 -->
    <input class="nui-hidden" name="criteria/_entity" value="com.cgn.itmp.systemset.BaseMgt.BaseMgt">
    <!-- 排序字段 -->
...
<div class="nui-panel" style="width:100%;height:100%;" showToolbar="false" showFooter="false">
    ...
    <div class="nui-fit">
        <div id="datagrid1" dataField="tddxgljddicts" class="nui-datagrid" style="width:100%;height:100%;"
             url="com.cgn.itmp.systemset.BaseMgt.queryTdDxglJddicts.biz.ext" pageSize="20" sizeList="[20,100,1000]" showPageInfo="true"
...
//     grid.load(formData);
 var grid = nui.get("datagrid1");
    ...
    function insertFile() {
        nui.open({
            showMaxButton: true,//http://127.0.0.1:8080/itmp/systemset/uploadfile.jsp
                ...
                function (action) {
                    if (action == "ok") {
                        ...
                        for(var i = 0 ; i < rows.length ; i++){
                            powerName += rows[i].name + (rows.length -1 == i ? "":",");
                        }
                        grid.loading("正在恢复中,请稍等...");
                        var json = "{'powerName':'"+ powerName +"',num:0}";
                        $.ajax({
                            ...
                            success: function (text) {
                                if (text.result) {
                                    ...
                                    json = nui.encode({tddxgljddicts: rows});
                                    $.ajax({
                                        url: "com.cgn.itmp.systemset.BaseMgt.hfDeleteTdDxglJddicts.biz.ext",
                                        type: 'POST',
                                        data: json,
                                        cache: false,
                                        contentType: 'text/json',
    ...
    function remove() {
        ...
        if (rows.length > 0) {
            nui.confirm("确定删除选中记录？", "系统提示",
                function (action) {
                    if (action == "ok") {
                        var json = nui.encode({tddxgljddicts: rows});
                        grid.loading("正在删除中,请稍等...");
                        $.ajax({
                            url: "com.cgn.itmp.systemset.BaseMgt.deleteTdDxglJddicts.biz.ext",
                            type: 'POST',
                            data: json,
                            cache: false,
                            contentType: 'text/json',
