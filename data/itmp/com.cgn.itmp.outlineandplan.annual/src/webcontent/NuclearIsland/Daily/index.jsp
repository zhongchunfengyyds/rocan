...
- Description:
    ...
    <body style="width:98%;height:97%;">
        <div class="nui-panel" title="年度计划核岛日常计划" style="width:100%;height:auto;" showToolbar="false" showFooter="true">
            <div id="form1" class="nui-form" align="left" style="height:100%">
                <!-- 数据实体的名称 -->
                <input class="nui-hidden" name="criteria/_entity" value="com.cgn.itmp.outlineandplan.annual.NuclearIslandDaily.PlanEntity">
                <!-- 排序字段 -->
        ...
        <div title="center" region="center" style="width:100%;height:90%;">
            <div class="nui-fit">
                <div 
                        id="datagrid1"
                        dataField="tgglndjhhdrcs"
                        class="nui-datagrid"
                        style="width:100%;height:100%;"
                        url="com.cgn.itmp.outlineandplan.annual.NuclearIslandDaily.queryTgglndjhhdrcs.biz.ext"
        ...
        <script type="text/javascript">
			...
			if(jdid != null && jdid != '' && jzid != null && jzid != ''){
				search();//grid查询
                                ...
                                function createplan(){
                                	...
                                	if(lc == ""){
                                		...
                                		json.lc = lc;
                                		json.ident = "年度计划核岛日常";
                                		json.entity = "com.cgn.itmp.outlineandplan.outline.common.curriculumVitae.Dgglvitae9";
                                		...
                                		$.ajax({
							                ...
							                success:function(data){
							                	if(data.record){
							                		var jsondata = '{"data":{"ident":"年度计划核岛日常","baseid":"'+jdid+'","crewid":"'+jzid+'","lc":"'+lc+'","username":"'+'<%= username%>'+'"}}';
			        								var urlStr = "com.cgn.itmp.outlineandplan.annual.NuclearIslandOverhaul.createNDJHRC.biz.ext";
							                		...
							                		nui.alert("该年度日常计划数据已存在,无需重新生成","提示");
							                	}
							                }
							            });
                                	}
					           ...
					           //超链接页面  
					            function showdetail() {
					                ...
					                if (row) {
					                    nui.open({showMaxButton : true,
					                        url: "/itmp/annual/NuclearIsland/Daily/linkList.jsp",
					                        title: "数据详情",
					                        width: 1000,
					                        height: 600,
					                        onload: function () {
			                    ...
			                    function showrecorddetail(){
					                ...
					                if (row) {
					                	nui.open({showMaxButton : true,
					                        url: "/itmp/annual/NuclearIsland/Daily/linkDetail.jsp",
					                        title: "数据详情",
					                        width: 800,
					                        height: 550,
					                        onload: function () {
					                             var iframe = this.getIFrameEl();
								                 var data = {pageType:"data",record:{ndplan:row.ndplan,crew:row.crew,base:row.base,lc:row.lc,ident:'年度计划核岛日常'}};
								                //直接从页面获取，不用去后台获取
								                iframe.contentWindow.setFormData(data);
				                            }
			                            });
									...
									if(rows.length > 0){
										...
										function(action){
								            if(action=="ok"){
								                ...
								                json.entity='com.cgn.itmp.outlineandplan.annual.NuclearIslandDaily.PlanEntity';
								                json.mxEntity='com.cgn.itmp.outlineandplan.annual.NuclearIslandDailyCreate.PlanGeneration';
				...
				function effect(){
						...
						if(row[0].isdelete == "4"){
							...
							nui.open({
							showMaxButton : true,
							 url : '/itmp/annual/NuclearIsland/Daily/effectList.jsp',
							 title :'生效',
					 		 width: '90%',
			                 height: '90%',
			                 onload: function () {
		 ...
		 function downloadFile(e){
          ...
          function expotyPDF(id){
         		var json = nui.encode({
         					id: id,
         					entityName:'com.cgn.itmp.outlineandplan.annual.NuclearIslandDaily.PlanEntity',
         					childEntityName:'com.cgn.itmp.outlineandplan.annual.NuclearIslandDailyCreate.PlanGeneration'
         				});
